//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     ANTLR Version: 3.5.2
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

// $ANTLR 3.5.2 Scrape.g 2016-01-30 20:53:07

// The variable 'variable' is assigned but its value is never used.
#pragma warning disable 219
// Unreachable code detected.
#pragma warning disable 162
// Missing XML comment for publicly visible type or member 'Type_or_Member'
#pragma warning disable 1591
// CLS compliance checking will not be performed on 'type' because it is not visible from outside this assembly.
#pragma warning disable 3019


using System.Collections.Generic;
using Antlr.Runtime;
using Antlr.Runtime.Misc;

[System.CodeDom.Compiler.GeneratedCode("ANTLR", "3.5.2")]
[System.CLSCompliant(false)]
public partial class ScrapeLexer : Antlr.Runtime.Lexer
{
	public const int EOF=-1;
	public const int T__110=110;
	public const int T__111=111;
	public const int AND=4;
	public const int APOSTRAPHE=5;
	public const int ASTERISK=6;
	public const int ATTRIBUTE=7;
	public const int BLOCK=8;
	public const int BUFFER=9;
	public const int BUFFER_TABLE=10;
	public const int CASE=11;
	public const int CASE_BOOL=12;
	public const int CASE_EXPRESSION=13;
	public const int CASE_VAR=14;
	public const int CLOSEBRACE=15;
	public const int CLOSEPAREN=16;
	public const int COMMA=17;
	public const int COMMENT=18;
	public const int CREATE=19;
	public const int DIGIT=20;
	public const int DIV=21;
	public const int DOT=22;
	public const int DOWNLOAD_IMAGE=23;
	public const int DOWNLOAD_PAGE=24;
	public const int EACH=25;
	public const int ELSE=26;
	public const int END=27;
	public const int EQUALS=28;
	public const int EXEC=29;
	public const int EXPAND=30;
	public const int EXPAND_INTERATION_VARIABLE=31;
	public const int FIELD_TERMINATOR=32;
	public const int FILE=33;
	public const int FILE_TABLE=34;
	public const int FLOAT=35;
	public const int FROM=36;
	public const int GREATERTHAN=37;
	public const int GREATERTHANEQUAL=38;
	public const int HTML=39;
	public const int ID=40;
	public const int IDENTITY=41;
	public const int IN=42;
	public const int INNER_JOIN=43;
	public const int INSERT_DIRECTORY=44;
	public const int INSERT_INTO=45;
	public const int INSERT_INTO_DIRECTORY=46;
	public const int INSERT_OVERWRITE=47;
	public const int INT=48;
	public const int INTEGER=49;
	public const int JOIN=50;
	public const int LESSTHAN=51;
	public const int LESSTHANEQUAL=52;
	public const int LETTER=53;
	public const int LINE_COMMENT_DASH=54;
	public const int LINE_COMMENT_SLASH=55;
	public const int LOCATION=56;
	public const int MATCH=57;
	public const int MEMBER_REFERENCE=58;
	public const int MINIS=59;
	public const int NEWLINE=60;
	public const int NODES=61;
	public const int NOTEQUAL=62;
	public const int NULL=63;
	public const int OPENBRACE=64;
	public const int OPENPAREN=65;
	public const int OR=66;
	public const int OTHERCHARS=67;
	public const int PICK=68;
	public const int PLUS=69;
	public const int POST_PICK=70;
	public const int PRE_PICK=71;
	public const int PROCEDURE=72;
	public const int PROGRAM=73;
	public const int PROXIES=74;
	public const int PROXY_LIST=75;
	public const int QUOTE=76;
	public const int REPLACE=77;
	public const int ROW_REFERENCE=78;
	public const int ROW_TERMINATOR=79;
	public const int SELECT=80;
	public const int SELECT_ALL=81;
	public const int SELECT_ARG=82;
	public const int SELECT_ID=83;
	public const int SELECT_STATEMENT=84;
	public const int STRING=85;
	public const int STRING_LITERAL=86;
	public const int TABLE_ALIAS=87;
	public const int TABLE_COLUMN_ARG=88;
	public const int TABLE_COLUMN_ARGS=89;
	public const int TABLE_MEMBER_REFERENCE=90;
	public const int TABLE_VARIABLE_REFERENCE=91;
	public const int TAKE=92;
	public const int TAKE_ATTRIBUTE=93;
	public const int TAKE_HTML=94;
	public const int TAKE_TEXT=95;
	public const int TEST=96;
	public const int TEXT=97;
	public const int THEN=98;
	public const int TO=99;
	public const int TRUNCATE=100;
	public const int VARIABLE=101;
	public const int VARIABLE_DECLARATION=102;
	public const int VARIABLE_REFERENCE=103;
	public const int WHEN=104;
	public const int WHEN_BOOL_STATEMENT=105;
	public const int WHEN_LITERAL_STATEMENT=106;
	public const int WHERE=107;
	public const int WITH=108;
	public const int WS=109;

	// delegates
	// delegators

	public ScrapeLexer()
	{
		OnCreated();
	}

	public ScrapeLexer(ICharStream input )
		: this(input, new RecognizerSharedState())
	{
	}

	public ScrapeLexer(ICharStream input, RecognizerSharedState state)
		: base(input, state)
	{

		OnCreated();
	}
	public override string GrammarFileName { get { return "Scrape.g"; } }


	partial void OnCreated();
	partial void EnterRule(string ruleName, int ruleIndex);
	partial void LeaveRule(string ruleName, int ruleIndex);

	partial void EnterRule_T__110();
	partial void LeaveRule_T__110();

	// $ANTLR start "T__110"
	[GrammarRule("T__110")]
	private void mT__110()
	{
		EnterRule_T__110();
		EnterRule("T__110", 1);
		TraceIn("T__110", 1);
		try
		{
			int _type = T__110;
			int _channel = DefaultTokenChannel;
			// Scrape.g:8:8: ( '$' )
			DebugEnterAlt(1);
			// Scrape.g:8:10: '$'
			{
			DebugLocation(8, 10);
			Match('$'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("T__110", 1);
			LeaveRule("T__110", 1);
			LeaveRule_T__110();
		}
	}
	// $ANTLR end "T__110"

	partial void EnterRule_T__111();
	partial void LeaveRule_T__111();

	// $ANTLR start "T__111"
	[GrammarRule("T__111")]
	private void mT__111()
	{
		EnterRule_T__111();
		EnterRule("T__111", 2);
		TraceIn("T__111", 2);
		try
		{
			int _type = T__111;
			int _channel = DefaultTokenChannel;
			// Scrape.g:9:8: ( 'on' )
			DebugEnterAlt(1);
			// Scrape.g:9:10: 'on'
			{
			DebugLocation(9, 10);
			Match("on"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("T__111", 2);
			LeaveRule("T__111", 2);
			LeaveRule_T__111();
		}
	}
	// $ANTLR end "T__111"

	partial void EnterRule_PROCEDURE();
	partial void LeaveRule_PROCEDURE();

	// $ANTLR start "PROCEDURE"
	[GrammarRule("PROCEDURE")]
	private void mPROCEDURE()
	{
		EnterRule_PROCEDURE();
		EnterRule("PROCEDURE", 3);
		TraceIn("PROCEDURE", 3);
		try
		{
			int _type = PROCEDURE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:346:10: ( 'procedure' )
			DebugEnterAlt(1);
			// Scrape.g:346:12: 'procedure'
			{
			DebugLocation(346, 12);
			Match("procedure"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("PROCEDURE", 3);
			LeaveRule("PROCEDURE", 3);
			LeaveRule_PROCEDURE();
		}
	}
	// $ANTLR end "PROCEDURE"

	partial void EnterRule_EXEC();
	partial void LeaveRule_EXEC();

	// $ANTLR start "EXEC"
	[GrammarRule("EXEC")]
	private void mEXEC()
	{
		EnterRule_EXEC();
		EnterRule("EXEC", 4);
		TraceIn("EXEC", 4);
		try
		{
			int _type = EXEC;
			int _channel = DefaultTokenChannel;
			// Scrape.g:347:5: ( 'exec' )
			DebugEnterAlt(1);
			// Scrape.g:347:7: 'exec'
			{
			DebugLocation(347, 7);
			Match("exec"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("EXEC", 4);
			LeaveRule("EXEC", 4);
			LeaveRule_EXEC();
		}
	}
	// $ANTLR end "EXEC"

	partial void EnterRule_CASE();
	partial void LeaveRule_CASE();

	// $ANTLR start "CASE"
	[GrammarRule("CASE")]
	private void mCASE()
	{
		EnterRule_CASE();
		EnterRule("CASE", 5);
		TraceIn("CASE", 5);
		try
		{
			int _type = CASE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:348:5: ( 'case' )
			DebugEnterAlt(1);
			// Scrape.g:348:7: 'case'
			{
			DebugLocation(348, 7);
			Match("case"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("CASE", 5);
			LeaveRule("CASE", 5);
			LeaveRule_CASE();
		}
	}
	// $ANTLR end "CASE"

	partial void EnterRule_WHEN();
	partial void LeaveRule_WHEN();

	// $ANTLR start "WHEN"
	[GrammarRule("WHEN")]
	private void mWHEN()
	{
		EnterRule_WHEN();
		EnterRule("WHEN", 6);
		TraceIn("WHEN", 6);
		try
		{
			int _type = WHEN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:349:5: ( 'when' )
			DebugEnterAlt(1);
			// Scrape.g:349:7: 'when'
			{
			DebugLocation(349, 7);
			Match("when"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("WHEN", 6);
			LeaveRule("WHEN", 6);
			LeaveRule_WHEN();
		}
	}
	// $ANTLR end "WHEN"

	partial void EnterRule_THEN();
	partial void LeaveRule_THEN();

	// $ANTLR start "THEN"
	[GrammarRule("THEN")]
	private void mTHEN()
	{
		EnterRule_THEN();
		EnterRule("THEN", 7);
		TraceIn("THEN", 7);
		try
		{
			int _type = THEN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:350:5: ( 'then' )
			DebugEnterAlt(1);
			// Scrape.g:350:7: 'then'
			{
			DebugLocation(350, 7);
			Match("then"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("THEN", 7);
			LeaveRule("THEN", 7);
			LeaveRule_THEN();
		}
	}
	// $ANTLR end "THEN"

	partial void EnterRule_END();
	partial void LeaveRule_END();

	// $ANTLR start "END"
	[GrammarRule("END")]
	private void mEND()
	{
		EnterRule_END();
		EnterRule("END", 8);
		TraceIn("END", 8);
		try
		{
			int _type = END;
			int _channel = DefaultTokenChannel;
			// Scrape.g:351:4: ( 'end' )
			DebugEnterAlt(1);
			// Scrape.g:351:6: 'end'
			{
			DebugLocation(351, 6);
			Match("end"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("END", 8);
			LeaveRule("END", 8);
			LeaveRule_END();
		}
	}
	// $ANTLR end "END"

	partial void EnterRule_ELSE();
	partial void LeaveRule_ELSE();

	// $ANTLR start "ELSE"
	[GrammarRule("ELSE")]
	private void mELSE()
	{
		EnterRule_ELSE();
		EnterRule("ELSE", 9);
		TraceIn("ELSE", 9);
		try
		{
			int _type = ELSE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:352:5: ( 'else' )
			DebugEnterAlt(1);
			// Scrape.g:352:7: 'else'
			{
			DebugLocation(352, 7);
			Match("else"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("ELSE", 9);
			LeaveRule("ELSE", 9);
			LeaveRule_ELSE();
		}
	}
	// $ANTLR end "ELSE"

	partial void EnterRule_AND();
	partial void LeaveRule_AND();

	// $ANTLR start "AND"
	[GrammarRule("AND")]
	private void mAND()
	{
		EnterRule_AND();
		EnterRule("AND", 10);
		TraceIn("AND", 10);
		try
		{
			int _type = AND;
			int _channel = DefaultTokenChannel;
			// Scrape.g:354:5: ( 'and' )
			DebugEnterAlt(1);
			// Scrape.g:354:7: 'and'
			{
			DebugLocation(354, 7);
			Match("and"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("AND", 10);
			LeaveRule("AND", 10);
			LeaveRule_AND();
		}
	}
	// $ANTLR end "AND"

	partial void EnterRule_OR();
	partial void LeaveRule_OR();

	// $ANTLR start "OR"
	[GrammarRule("OR")]
	private void mOR()
	{
		EnterRule_OR();
		EnterRule("OR", 11);
		TraceIn("OR", 11);
		try
		{
			int _type = OR;
			int _channel = DefaultTokenChannel;
			// Scrape.g:355:4: ( 'or' )
			DebugEnterAlt(1);
			// Scrape.g:355:6: 'or'
			{
			DebugLocation(355, 6);
			Match("or"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("OR", 11);
			LeaveRule("OR", 11);
			LeaveRule_OR();
		}
	}
	// $ANTLR end "OR"

	partial void EnterRule_EQUALS();
	partial void LeaveRule_EQUALS();

	// $ANTLR start "EQUALS"
	[GrammarRule("EQUALS")]
	private void mEQUALS()
	{
		EnterRule_EQUALS();
		EnterRule("EQUALS", 12);
		TraceIn("EQUALS", 12);
		try
		{
			int _type = EQUALS;
			int _channel = DefaultTokenChannel;
			// Scrape.g:357:8: ( '=' )
			DebugEnterAlt(1);
			// Scrape.g:357:10: '='
			{
			DebugLocation(357, 10);
			Match('='); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("EQUALS", 12);
			LeaveRule("EQUALS", 12);
			LeaveRule_EQUALS();
		}
	}
	// $ANTLR end "EQUALS"

	partial void EnterRule_LESSTHAN();
	partial void LeaveRule_LESSTHAN();

	// $ANTLR start "LESSTHAN"
	[GrammarRule("LESSTHAN")]
	private void mLESSTHAN()
	{
		EnterRule_LESSTHAN();
		EnterRule("LESSTHAN", 13);
		TraceIn("LESSTHAN", 13);
		try
		{
			int _type = LESSTHAN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:358:10: ( '<' )
			DebugEnterAlt(1);
			// Scrape.g:358:12: '<'
			{
			DebugLocation(358, 12);
			Match('<'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("LESSTHAN", 13);
			LeaveRule("LESSTHAN", 13);
			LeaveRule_LESSTHAN();
		}
	}
	// $ANTLR end "LESSTHAN"

	partial void EnterRule_LESSTHANEQUAL();
	partial void LeaveRule_LESSTHANEQUAL();

	// $ANTLR start "LESSTHANEQUAL"
	[GrammarRule("LESSTHANEQUAL")]
	private void mLESSTHANEQUAL()
	{
		EnterRule_LESSTHANEQUAL();
		EnterRule("LESSTHANEQUAL", 14);
		TraceIn("LESSTHANEQUAL", 14);
		try
		{
			int _type = LESSTHANEQUAL;
			int _channel = DefaultTokenChannel;
			// Scrape.g:359:14: ( '<=' )
			DebugEnterAlt(1);
			// Scrape.g:359:16: '<='
			{
			DebugLocation(359, 16);
			Match("<="); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("LESSTHANEQUAL", 14);
			LeaveRule("LESSTHANEQUAL", 14);
			LeaveRule_LESSTHANEQUAL();
		}
	}
	// $ANTLR end "LESSTHANEQUAL"

	partial void EnterRule_GREATERTHAN();
	partial void LeaveRule_GREATERTHAN();

	// $ANTLR start "GREATERTHAN"
	[GrammarRule("GREATERTHAN")]
	private void mGREATERTHAN()
	{
		EnterRule_GREATERTHAN();
		EnterRule("GREATERTHAN", 15);
		TraceIn("GREATERTHAN", 15);
		try
		{
			int _type = GREATERTHAN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:360:12: ( '>' )
			DebugEnterAlt(1);
			// Scrape.g:360:14: '>'
			{
			DebugLocation(360, 14);
			Match('>'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("GREATERTHAN", 15);
			LeaveRule("GREATERTHAN", 15);
			LeaveRule_GREATERTHAN();
		}
	}
	// $ANTLR end "GREATERTHAN"

	partial void EnterRule_GREATERTHANEQUAL();
	partial void LeaveRule_GREATERTHANEQUAL();

	// $ANTLR start "GREATERTHANEQUAL"
	[GrammarRule("GREATERTHANEQUAL")]
	private void mGREATERTHANEQUAL()
	{
		EnterRule_GREATERTHANEQUAL();
		EnterRule("GREATERTHANEQUAL", 16);
		TraceIn("GREATERTHANEQUAL", 16);
		try
		{
			int _type = GREATERTHANEQUAL;
			int _channel = DefaultTokenChannel;
			// Scrape.g:361:17: ( '>=' )
			DebugEnterAlt(1);
			// Scrape.g:361:19: '>='
			{
			DebugLocation(361, 19);
			Match(">="); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("GREATERTHANEQUAL", 16);
			LeaveRule("GREATERTHANEQUAL", 16);
			LeaveRule_GREATERTHANEQUAL();
		}
	}
	// $ANTLR end "GREATERTHANEQUAL"

	partial void EnterRule_NOTEQUAL();
	partial void LeaveRule_NOTEQUAL();

	// $ANTLR start "NOTEQUAL"
	[GrammarRule("NOTEQUAL")]
	private void mNOTEQUAL()
	{
		EnterRule_NOTEQUAL();
		EnterRule("NOTEQUAL", 17);
		TraceIn("NOTEQUAL", 17);
		try
		{
			int _type = NOTEQUAL;
			int _channel = DefaultTokenChannel;
			// Scrape.g:362:9: ( '!=' )
			DebugEnterAlt(1);
			// Scrape.g:362:11: '!='
			{
			DebugLocation(362, 11);
			Match("!="); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("NOTEQUAL", 17);
			LeaveRule("NOTEQUAL", 17);
			LeaveRule_NOTEQUAL();
		}
	}
	// $ANTLR end "NOTEQUAL"

	partial void EnterRule_INSERT_INTO();
	partial void LeaveRule_INSERT_INTO();

	// $ANTLR start "INSERT_INTO"
	[GrammarRule("INSERT_INTO")]
	private void mINSERT_INTO()
	{
		EnterRule_INSERT_INTO();
		EnterRule("INSERT_INTO", 18);
		TraceIn("INSERT_INTO", 18);
		try
		{
			int _type = INSERT_INTO;
			int _channel = DefaultTokenChannel;
			// Scrape.g:364:13: ( 'insert into' )
			DebugEnterAlt(1);
			// Scrape.g:364:15: 'insert into'
			{
			DebugLocation(364, 15);
			Match("insert into"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("INSERT_INTO", 18);
			LeaveRule("INSERT_INTO", 18);
			LeaveRule_INSERT_INTO();
		}
	}
	// $ANTLR end "INSERT_INTO"

	partial void EnterRule_INSERT_OVERWRITE();
	partial void LeaveRule_INSERT_OVERWRITE();

	// $ANTLR start "INSERT_OVERWRITE"
	[GrammarRule("INSERT_OVERWRITE")]
	private void mINSERT_OVERWRITE()
	{
		EnterRule_INSERT_OVERWRITE();
		EnterRule("INSERT_OVERWRITE", 19);
		TraceIn("INSERT_OVERWRITE", 19);
		try
		{
			int _type = INSERT_OVERWRITE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:365:18: ( 'insert overwrite' )
			DebugEnterAlt(1);
			// Scrape.g:365:20: 'insert overwrite'
			{
			DebugLocation(365, 20);
			Match("insert overwrite"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("INSERT_OVERWRITE", 19);
			LeaveRule("INSERT_OVERWRITE", 19);
			LeaveRule_INSERT_OVERWRITE();
		}
	}
	// $ANTLR end "INSERT_OVERWRITE"

	partial void EnterRule_INSERT_DIRECTORY();
	partial void LeaveRule_INSERT_DIRECTORY();

	// $ANTLR start "INSERT_DIRECTORY"
	[GrammarRule("INSERT_DIRECTORY")]
	private void mINSERT_DIRECTORY()
	{
		EnterRule_INSERT_DIRECTORY();
		EnterRule("INSERT_DIRECTORY", 20);
		TraceIn("INSERT_DIRECTORY", 20);
		try
		{
			int _type = INSERT_DIRECTORY;
			int _channel = DefaultTokenChannel;
			// Scrape.g:366:18: ( 'insert file into' )
			DebugEnterAlt(1);
			// Scrape.g:366:20: 'insert file into'
			{
			DebugLocation(366, 20);
			Match("insert file into"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("INSERT_DIRECTORY", 20);
			LeaveRule("INSERT_DIRECTORY", 20);
			LeaveRule_INSERT_DIRECTORY();
		}
	}
	// $ANTLR end "INSERT_DIRECTORY"

	partial void EnterRule_TRUNCATE();
	partial void LeaveRule_TRUNCATE();

	// $ANTLR start "TRUNCATE"
	[GrammarRule("TRUNCATE")]
	private void mTRUNCATE()
	{
		EnterRule_TRUNCATE();
		EnterRule("TRUNCATE", 21);
		TraceIn("TRUNCATE", 21);
		try
		{
			int _type = TRUNCATE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:367:10: ( 'truncate' )
			DebugEnterAlt(1);
			// Scrape.g:367:12: 'truncate'
			{
			DebugLocation(367, 12);
			Match("truncate"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("TRUNCATE", 21);
			LeaveRule("TRUNCATE", 21);
			LeaveRule_TRUNCATE();
		}
	}
	// $ANTLR end "TRUNCATE"

	partial void EnterRule_EACH();
	partial void LeaveRule_EACH();

	// $ANTLR start "EACH"
	[GrammarRule("EACH")]
	private void mEACH()
	{
		EnterRule_EACH();
		EnterRule("EACH", 22);
		TraceIn("EACH", 22);
		try
		{
			int _type = EACH;
			int _channel = DefaultTokenChannel;
			// Scrape.g:368:6: ( 'each' )
			DebugEnterAlt(1);
			// Scrape.g:368:8: 'each'
			{
			DebugLocation(368, 8);
			Match("each"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("EACH", 22);
			LeaveRule("EACH", 22);
			LeaveRule_EACH();
		}
	}
	// $ANTLR end "EACH"

	partial void EnterRule_IN();
	partial void LeaveRule_IN();

	// $ANTLR start "IN"
	[GrammarRule("IN")]
	private void mIN()
	{
		EnterRule_IN();
		EnterRule("IN", 23);
		TraceIn("IN", 23);
		try
		{
			int _type = IN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:369:4: ( 'in' )
			DebugEnterAlt(1);
			// Scrape.g:369:6: 'in'
			{
			DebugLocation(369, 6);
			Match("in"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("IN", 23);
			LeaveRule("IN", 23);
			LeaveRule_IN();
		}
	}
	// $ANTLR end "IN"

	partial void EnterRule_SELECT();
	partial void LeaveRule_SELECT();

	// $ANTLR start "SELECT"
	[GrammarRule("SELECT")]
	private void mSELECT()
	{
		EnterRule_SELECT();
		EnterRule("SELECT", 24);
		TraceIn("SELECT", 24);
		try
		{
			int _type = SELECT;
			int _channel = DefaultTokenChannel;
			// Scrape.g:370:8: ( 'select' )
			DebugEnterAlt(1);
			// Scrape.g:370:10: 'select'
			{
			DebugLocation(370, 10);
			Match("select"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("SELECT", 24);
			LeaveRule("SELECT", 24);
			LeaveRule_SELECT();
		}
	}
	// $ANTLR end "SELECT"

	partial void EnterRule_FROM();
	partial void LeaveRule_FROM();

	// $ANTLR start "FROM"
	[GrammarRule("FROM")]
	private void mFROM()
	{
		EnterRule_FROM();
		EnterRule("FROM", 25);
		TraceIn("FROM", 25);
		try
		{
			int _type = FROM;
			int _channel = DefaultTokenChannel;
			// Scrape.g:371:6: ( 'from' )
			DebugEnterAlt(1);
			// Scrape.g:371:8: 'from'
			{
			DebugLocation(371, 8);
			Match("from"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("FROM", 25);
			LeaveRule("FROM", 25);
			LeaveRule_FROM();
		}
	}
	// $ANTLR end "FROM"

	partial void EnterRule_INNER_JOIN();
	partial void LeaveRule_INNER_JOIN();

	// $ANTLR start "INNER_JOIN"
	[GrammarRule("INNER_JOIN")]
	private void mINNER_JOIN()
	{
		EnterRule_INNER_JOIN();
		EnterRule("INNER_JOIN", 26);
		TraceIn("INNER_JOIN", 26);
		try
		{
			int _type = INNER_JOIN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:372:12: ( 'inner join' )
			DebugEnterAlt(1);
			// Scrape.g:372:14: 'inner join'
			{
			DebugLocation(372, 14);
			Match("inner join"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("INNER_JOIN", 26);
			LeaveRule("INNER_JOIN", 26);
			LeaveRule_INNER_JOIN();
		}
	}
	// $ANTLR end "INNER_JOIN"

	partial void EnterRule_JOIN();
	partial void LeaveRule_JOIN();

	// $ANTLR start "JOIN"
	[GrammarRule("JOIN")]
	private void mJOIN()
	{
		EnterRule_JOIN();
		EnterRule("JOIN", 27);
		TraceIn("JOIN", 27);
		try
		{
			int _type = JOIN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:373:5: ( 'join' )
			DebugEnterAlt(1);
			// Scrape.g:373:7: 'join'
			{
			DebugLocation(373, 7);
			Match("join"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("JOIN", 27);
			LeaveRule("JOIN", 27);
			LeaveRule_JOIN();
		}
	}
	// $ANTLR end "JOIN"

	partial void EnterRule_NODES();
	partial void LeaveRule_NODES();

	// $ANTLR start "NODES"
	[GrammarRule("NODES")]
	private void mNODES()
	{
		EnterRule_NODES();
		EnterRule("NODES", 28);
		TraceIn("NODES", 28);
		try
		{
			int _type = NODES;
			int _channel = DefaultTokenChannel;
			// Scrape.g:374:6: ( 'nodes' )
			DebugEnterAlt(1);
			// Scrape.g:374:8: 'nodes'
			{
			DebugLocation(374, 8);
			Match("nodes"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("NODES", 28);
			LeaveRule("NODES", 28);
			LeaveRule_NODES();
		}
	}
	// $ANTLR end "NODES"

	partial void EnterRule_WHERE();
	partial void LeaveRule_WHERE();

	// $ANTLR start "WHERE"
	[GrammarRule("WHERE")]
	private void mWHERE()
	{
		EnterRule_WHERE();
		EnterRule("WHERE", 29);
		TraceIn("WHERE", 29);
		try
		{
			int _type = WHERE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:375:7: ( 'where' )
			DebugEnterAlt(1);
			// Scrape.g:375:9: 'where'
			{
			DebugLocation(375, 9);
			Match("where"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("WHERE", 29);
			LeaveRule("WHERE", 29);
			LeaveRule_WHERE();
		}
	}
	// $ANTLR end "WHERE"

	partial void EnterRule_PICK();
	partial void LeaveRule_PICK();

	// $ANTLR start "PICK"
	[GrammarRule("PICK")]
	private void mPICK()
	{
		EnterRule_PICK();
		EnterRule("PICK", 30);
		TraceIn("PICK", 30);
		try
		{
			int _type = PICK;
			int _channel = DefaultTokenChannel;
			// Scrape.g:376:6: ( 'pick' )
			DebugEnterAlt(1);
			// Scrape.g:376:8: 'pick'
			{
			DebugLocation(376, 8);
			Match("pick"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("PICK", 30);
			LeaveRule("PICK", 30);
			LeaveRule_PICK();
		}
	}
	// $ANTLR end "PICK"

	partial void EnterRule_TAKE();
	partial void LeaveRule_TAKE();

	// $ANTLR start "TAKE"
	[GrammarRule("TAKE")]
	private void mTAKE()
	{
		EnterRule_TAKE();
		EnterRule("TAKE", 31);
		TraceIn("TAKE", 31);
		try
		{
			int _type = TAKE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:377:6: ( 'take' )
			DebugEnterAlt(1);
			// Scrape.g:377:8: 'take'
			{
			DebugLocation(377, 8);
			Match("take"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("TAKE", 31);
			LeaveRule("TAKE", 31);
			LeaveRule_TAKE();
		}
	}
	// $ANTLR end "TAKE"

	partial void EnterRule_MATCH();
	partial void LeaveRule_MATCH();

	// $ANTLR start "MATCH"
	[GrammarRule("MATCH")]
	private void mMATCH()
	{
		EnterRule_MATCH();
		EnterRule("MATCH", 32);
		TraceIn("MATCH", 32);
		try
		{
			int _type = MATCH;
			int _channel = DefaultTokenChannel;
			// Scrape.g:378:7: ( 'match' )
			DebugEnterAlt(1);
			// Scrape.g:378:9: 'match'
			{
			DebugLocation(378, 9);
			Match("match"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("MATCH", 32);
			LeaveRule("MATCH", 32);
			LeaveRule_MATCH();
		}
	}
	// $ANTLR end "MATCH"

	partial void EnterRule_REPLACE();
	partial void LeaveRule_REPLACE();

	// $ANTLR start "REPLACE"
	[GrammarRule("REPLACE")]
	private void mREPLACE()
	{
		EnterRule_REPLACE();
		EnterRule("REPLACE", 33);
		TraceIn("REPLACE", 33);
		try
		{
			int _type = REPLACE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:379:9: ( 'replace' )
			DebugEnterAlt(1);
			// Scrape.g:379:11: 'replace'
			{
			DebugLocation(379, 11);
			Match("replace"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("REPLACE", 33);
			LeaveRule("REPLACE", 33);
			LeaveRule_REPLACE();
		}
	}
	// $ANTLR end "REPLACE"

	partial void EnterRule_ATTRIBUTE();
	partial void LeaveRule_ATTRIBUTE();

	// $ANTLR start "ATTRIBUTE"
	[GrammarRule("ATTRIBUTE")]
	private void mATTRIBUTE()
	{
		EnterRule_ATTRIBUTE();
		EnterRule("ATTRIBUTE", 34);
		TraceIn("ATTRIBUTE", 34);
		try
		{
			int _type = ATTRIBUTE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:380:11: ( 'attribute' )
			DebugEnterAlt(1);
			// Scrape.g:380:13: 'attribute'
			{
			DebugLocation(380, 13);
			Match("attribute"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("ATTRIBUTE", 34);
			LeaveRule("ATTRIBUTE", 34);
			LeaveRule_ATTRIBUTE();
		}
	}
	// $ANTLR end "ATTRIBUTE"

	partial void EnterRule_TEXT();
	partial void LeaveRule_TEXT();

	// $ANTLR start "TEXT"
	[GrammarRule("TEXT")]
	private void mTEXT()
	{
		EnterRule_TEXT();
		EnterRule("TEXT", 35);
		TraceIn("TEXT", 35);
		try
		{
			int _type = TEXT;
			int _channel = DefaultTokenChannel;
			// Scrape.g:381:6: ( 'text' )
			DebugEnterAlt(1);
			// Scrape.g:381:8: 'text'
			{
			DebugLocation(381, 8);
			Match("text"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("TEXT", 35);
			LeaveRule("TEXT", 35);
			LeaveRule_TEXT();
		}
	}
	// $ANTLR end "TEXT"

	partial void EnterRule_HTML();
	partial void LeaveRule_HTML();

	// $ANTLR start "HTML"
	[GrammarRule("HTML")]
	private void mHTML()
	{
		EnterRule_HTML();
		EnterRule("HTML", 36);
		TraceIn("HTML", 36);
		try
		{
			int _type = HTML;
			int _channel = DefaultTokenChannel;
			// Scrape.g:382:6: ( 'html' )
			DebugEnterAlt(1);
			// Scrape.g:382:8: 'html'
			{
			DebugLocation(382, 8);
			Match("html"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("HTML", 36);
			LeaveRule("HTML", 36);
			LeaveRule_HTML();
		}
	}
	// $ANTLR end "HTML"

	partial void EnterRule_DOWNLOAD_IMAGE();
	partial void LeaveRule_DOWNLOAD_IMAGE();

	// $ANTLR start "DOWNLOAD_IMAGE"
	[GrammarRule("DOWNLOAD_IMAGE")]
	private void mDOWNLOAD_IMAGE()
	{
		EnterRule_DOWNLOAD_IMAGE();
		EnterRule("DOWNLOAD_IMAGE", 37);
		TraceIn("DOWNLOAD_IMAGE", 37);
		try
		{
			int _type = DOWNLOAD_IMAGE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:383:16: ( 'download image' )
			DebugEnterAlt(1);
			// Scrape.g:383:18: 'download image'
			{
			DebugLocation(383, 18);
			Match("download image"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("DOWNLOAD_IMAGE", 37);
			LeaveRule("DOWNLOAD_IMAGE", 37);
			LeaveRule_DOWNLOAD_IMAGE();
		}
	}
	// $ANTLR end "DOWNLOAD_IMAGE"

	partial void EnterRule_DOWNLOAD_PAGE();
	partial void LeaveRule_DOWNLOAD_PAGE();

	// $ANTLR start "DOWNLOAD_PAGE"
	[GrammarRule("DOWNLOAD_PAGE")]
	private void mDOWNLOAD_PAGE()
	{
		EnterRule_DOWNLOAD_PAGE();
		EnterRule("DOWNLOAD_PAGE", 38);
		TraceIn("DOWNLOAD_PAGE", 38);
		try
		{
			int _type = DOWNLOAD_PAGE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:384:15: ( 'download page' )
			DebugEnterAlt(1);
			// Scrape.g:384:17: 'download page'
			{
			DebugLocation(384, 17);
			Match("download page"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("DOWNLOAD_PAGE", 38);
			LeaveRule("DOWNLOAD_PAGE", 38);
			LeaveRule_DOWNLOAD_PAGE();
		}
	}
	// $ANTLR end "DOWNLOAD_PAGE"

	partial void EnterRule_IDENTITY();
	partial void LeaveRule_IDENTITY();

	// $ANTLR start "IDENTITY"
	[GrammarRule("IDENTITY")]
	private void mIDENTITY()
	{
		EnterRule_IDENTITY();
		EnterRule("IDENTITY", 39);
		TraceIn("IDENTITY", 39);
		try
		{
			int _type = IDENTITY;
			int _channel = DefaultTokenChannel;
			// Scrape.g:385:10: ( 'identity' )
			DebugEnterAlt(1);
			// Scrape.g:385:12: 'identity'
			{
			DebugLocation(385, 12);
			Match("identity"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("IDENTITY", 39);
			LeaveRule("IDENTITY", 39);
			LeaveRule_IDENTITY();
		}
	}
	// $ANTLR end "IDENTITY"

	partial void EnterRule_EXPAND();
	partial void LeaveRule_EXPAND();

	// $ANTLR start "EXPAND"
	[GrammarRule("EXPAND")]
	private void mEXPAND()
	{
		EnterRule_EXPAND();
		EnterRule("EXPAND", 40);
		TraceIn("EXPAND", 40);
		try
		{
			int _type = EXPAND;
			int _channel = DefaultTokenChannel;
			// Scrape.g:386:8: ( 'expand' )
			DebugEnterAlt(1);
			// Scrape.g:386:10: 'expand'
			{
			DebugLocation(386, 10);
			Match("expand"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("EXPAND", 40);
			LeaveRule("EXPAND", 40);
			LeaveRule_EXPAND();
		}
	}
	// $ANTLR end "EXPAND"

	partial void EnterRule_TO();
	partial void LeaveRule_TO();

	// $ANTLR start "TO"
	[GrammarRule("TO")]
	private void mTO()
	{
		EnterRule_TO();
		EnterRule("TO", 41);
		TraceIn("TO", 41);
		try
		{
			int _type = TO;
			int _channel = DefaultTokenChannel;
			// Scrape.g:387:4: ( 'to' )
			DebugEnterAlt(1);
			// Scrape.g:387:6: 'to'
			{
			DebugLocation(387, 6);
			Match("to"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("TO", 41);
			LeaveRule("TO", 41);
			LeaveRule_TO();
		}
	}
	// $ANTLR end "TO"

	partial void EnterRule_PROXIES();
	partial void LeaveRule_PROXIES();

	// $ANTLR start "PROXIES"
	[GrammarRule("PROXIES")]
	private void mPROXIES()
	{
		EnterRule_PROXIES();
		EnterRule("PROXIES", 42);
		TraceIn("PROXIES", 42);
		try
		{
			int _type = PROXIES;
			int _channel = DefaultTokenChannel;
			// Scrape.g:388:8: ( 'proxies' )
			DebugEnterAlt(1);
			// Scrape.g:388:10: 'proxies'
			{
			DebugLocation(388, 10);
			Match("proxies"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("PROXIES", 42);
			LeaveRule("PROXIES", 42);
			LeaveRule_PROXIES();
		}
	}
	// $ANTLR end "PROXIES"

	partial void EnterRule_TEST();
	partial void LeaveRule_TEST();

	// $ANTLR start "TEST"
	[GrammarRule("TEST")]
	private void mTEST()
	{
		EnterRule_TEST();
		EnterRule("TEST", 43);
		TraceIn("TEST", 43);
		try
		{
			int _type = TEST;
			int _channel = DefaultTokenChannel;
			// Scrape.g:389:6: ( 'test' )
			DebugEnterAlt(1);
			// Scrape.g:389:8: 'test'
			{
			DebugLocation(389, 8);
			Match("test"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("TEST", 43);
			LeaveRule("TEST", 43);
			LeaveRule_TEST();
		}
	}
	// $ANTLR end "TEST"

	partial void EnterRule_NULL();
	partial void LeaveRule_NULL();

	// $ANTLR start "NULL"
	[GrammarRule("NULL")]
	private void mNULL()
	{
		EnterRule_NULL();
		EnterRule("NULL", 44);
		TraceIn("NULL", 44);
		try
		{
			int _type = NULL;
			int _channel = DefaultTokenChannel;
			// Scrape.g:390:6: ( 'null' )
			DebugEnterAlt(1);
			// Scrape.g:390:8: 'null'
			{
			DebugLocation(390, 8);
			Match("null"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("NULL", 44);
			LeaveRule("NULL", 44);
			LeaveRule_NULL();
		}
	}
	// $ANTLR end "NULL"

	partial void EnterRule_CREATE();
	partial void LeaveRule_CREATE();

	// $ANTLR start "CREATE"
	[GrammarRule("CREATE")]
	private void mCREATE()
	{
		EnterRule_CREATE();
		EnterRule("CREATE", 45);
		TraceIn("CREATE", 45);
		try
		{
			int _type = CREATE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:392:8: ( 'create' )
			DebugEnterAlt(1);
			// Scrape.g:392:10: 'create'
			{
			DebugLocation(392, 10);
			Match("create"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("CREATE", 45);
			LeaveRule("CREATE", 45);
			LeaveRule_CREATE();
		}
	}
	// $ANTLR end "CREATE"

	partial void EnterRule_FILE();
	partial void LeaveRule_FILE();

	// $ANTLR start "FILE"
	[GrammarRule("FILE")]
	private void mFILE()
	{
		EnterRule_FILE();
		EnterRule("FILE", 46);
		TraceIn("FILE", 46);
		try
		{
			int _type = FILE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:393:6: ( 'file' )
			DebugEnterAlt(1);
			// Scrape.g:393:8: 'file'
			{
			DebugLocation(393, 8);
			Match("file"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("FILE", 46);
			LeaveRule("FILE", 46);
			LeaveRule_FILE();
		}
	}
	// $ANTLR end "FILE"

	partial void EnterRule_BUFFER();
	partial void LeaveRule_BUFFER();

	// $ANTLR start "BUFFER"
	[GrammarRule("BUFFER")]
	private void mBUFFER()
	{
		EnterRule_BUFFER();
		EnterRule("BUFFER", 47);
		TraceIn("BUFFER", 47);
		try
		{
			int _type = BUFFER;
			int _channel = DefaultTokenChannel;
			// Scrape.g:394:8: ( 'buffer' )
			DebugEnterAlt(1);
			// Scrape.g:394:10: 'buffer'
			{
			DebugLocation(394, 10);
			Match("buffer"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("BUFFER", 47);
			LeaveRule("BUFFER", 47);
			LeaveRule_BUFFER();
		}
	}
	// $ANTLR end "BUFFER"

	partial void EnterRule_WITH();
	partial void LeaveRule_WITH();

	// $ANTLR start "WITH"
	[GrammarRule("WITH")]
	private void mWITH()
	{
		EnterRule_WITH();
		EnterRule("WITH", 48);
		TraceIn("WITH", 48);
		try
		{
			int _type = WITH;
			int _channel = DefaultTokenChannel;
			// Scrape.g:395:6: ( 'with' )
			DebugEnterAlt(1);
			// Scrape.g:395:8: 'with'
			{
			DebugLocation(395, 8);
			Match("with"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("WITH", 48);
			LeaveRule("WITH", 48);
			LeaveRule_WITH();
		}
	}
	// $ANTLR end "WITH"

	partial void EnterRule_STRING();
	partial void LeaveRule_STRING();

	// $ANTLR start "STRING"
	[GrammarRule("STRING")]
	private void mSTRING()
	{
		EnterRule_STRING();
		EnterRule("STRING", 49);
		TraceIn("STRING", 49);
		try
		{
			int _type = STRING;
			int _channel = DefaultTokenChannel;
			// Scrape.g:396:8: ( 'string' )
			DebugEnterAlt(1);
			// Scrape.g:396:10: 'string'
			{
			DebugLocation(396, 10);
			Match("string"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("STRING", 49);
			LeaveRule("STRING", 49);
			LeaveRule_STRING();
		}
	}
	// $ANTLR end "STRING"

	partial void EnterRule_INTEGER();
	partial void LeaveRule_INTEGER();

	// $ANTLR start "INTEGER"
	[GrammarRule("INTEGER")]
	private void mINTEGER()
	{
		EnterRule_INTEGER();
		EnterRule("INTEGER", 50);
		TraceIn("INTEGER", 50);
		try
		{
			int _type = INTEGER;
			int _channel = DefaultTokenChannel;
			// Scrape.g:397:8: ( 'int' )
			DebugEnterAlt(1);
			// Scrape.g:397:10: 'int'
			{
			DebugLocation(397, 10);
			Match("int"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("INTEGER", 50);
			LeaveRule("INTEGER", 50);
			LeaveRule_INTEGER();
		}
	}
	// $ANTLR end "INTEGER"

	partial void EnterRule_FLOAT();
	partial void LeaveRule_FLOAT();

	// $ANTLR start "FLOAT"
	[GrammarRule("FLOAT")]
	private void mFLOAT()
	{
		EnterRule_FLOAT();
		EnterRule("FLOAT", 51);
		TraceIn("FLOAT", 51);
		try
		{
			int _type = FLOAT;
			int _channel = DefaultTokenChannel;
			// Scrape.g:398:6: ( 'float' )
			DebugEnterAlt(1);
			// Scrape.g:398:8: 'float'
			{
			DebugLocation(398, 8);
			Match("float"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("FLOAT", 51);
			LeaveRule("FLOAT", 51);
			LeaveRule_FLOAT();
		}
	}
	// $ANTLR end "FLOAT"

	partial void EnterRule_FIELD_TERMINATOR();
	partial void LeaveRule_FIELD_TERMINATOR();

	// $ANTLR start "FIELD_TERMINATOR"
	[GrammarRule("FIELD_TERMINATOR")]
	private void mFIELD_TERMINATOR()
	{
		EnterRule_FIELD_TERMINATOR();
		EnterRule("FIELD_TERMINATOR", 52);
		TraceIn("FIELD_TERMINATOR", 52);
		try
		{
			int _type = FIELD_TERMINATOR;
			int _channel = DefaultTokenChannel;
			// Scrape.g:399:18: ( 'fieldterminator' )
			DebugEnterAlt(1);
			// Scrape.g:399:20: 'fieldterminator'
			{
			DebugLocation(399, 20);
			Match("fieldterminator"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("FIELD_TERMINATOR", 52);
			LeaveRule("FIELD_TERMINATOR", 52);
			LeaveRule_FIELD_TERMINATOR();
		}
	}
	// $ANTLR end "FIELD_TERMINATOR"

	partial void EnterRule_ROW_TERMINATOR();
	partial void LeaveRule_ROW_TERMINATOR();

	// $ANTLR start "ROW_TERMINATOR"
	[GrammarRule("ROW_TERMINATOR")]
	private void mROW_TERMINATOR()
	{
		EnterRule_ROW_TERMINATOR();
		EnterRule("ROW_TERMINATOR", 53);
		TraceIn("ROW_TERMINATOR", 53);
		try
		{
			int _type = ROW_TERMINATOR;
			int _channel = DefaultTokenChannel;
			// Scrape.g:400:16: ( 'rowterminator' )
			DebugEnterAlt(1);
			// Scrape.g:400:18: 'rowterminator'
			{
			DebugLocation(400, 18);
			Match("rowterminator"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("ROW_TERMINATOR", 53);
			LeaveRule("ROW_TERMINATOR", 53);
			LeaveRule_ROW_TERMINATOR();
		}
	}
	// $ANTLR end "ROW_TERMINATOR"

	partial void EnterRule_LOCATION();
	partial void LeaveRule_LOCATION();

	// $ANTLR start "LOCATION"
	[GrammarRule("LOCATION")]
	private void mLOCATION()
	{
		EnterRule_LOCATION();
		EnterRule("LOCATION", 54);
		TraceIn("LOCATION", 54);
		try
		{
			int _type = LOCATION;
			int _channel = DefaultTokenChannel;
			// Scrape.g:401:10: ( 'location' )
			DebugEnterAlt(1);
			// Scrape.g:401:12: 'location'
			{
			DebugLocation(401, 12);
			Match("location"); 


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("LOCATION", 54);
			LeaveRule("LOCATION", 54);
			LeaveRule_LOCATION();
		}
	}
	// $ANTLR end "LOCATION"

	partial void EnterRule_STRING_LITERAL();
	partial void LeaveRule_STRING_LITERAL();

	// $ANTLR start "STRING_LITERAL"
	[GrammarRule("STRING_LITERAL")]
	private void mSTRING_LITERAL()
	{
		EnterRule_STRING_LITERAL();
		EnterRule("STRING_LITERAL", 55);
		TraceIn("STRING_LITERAL", 55);
		try
		{
			int _type = STRING_LITERAL;
			int _channel = DefaultTokenChannel;
			// Scrape.g:403:15: ( APOSTRAPHE (~ ( APOSTRAPHE ) )* APOSTRAPHE )
			DebugEnterAlt(1);
			// Scrape.g:403:17: APOSTRAPHE (~ ( APOSTRAPHE ) )* APOSTRAPHE
			{
			DebugLocation(403, 17);
			mAPOSTRAPHE(); 
			DebugLocation(403, 28);
			// Scrape.g:403:28: (~ ( APOSTRAPHE ) )*
			try { DebugEnterSubRule(1);
			while (true)
			{
				int alt1=2;
				try { DebugEnterDecision(1, false);
				int LA1_1 = input.LA(1);

				if (((LA1_1>='\u0000' && LA1_1<='&')||(LA1_1>='(' && LA1_1<='\uFFFF')))
				{
					alt1 = 1;
				}


				} finally { DebugExitDecision(1); }
				switch ( alt1 )
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:
					{
					DebugLocation(403, 28);
					input.Consume();


					}
					break;

				default:
					goto loop1;
				}
			}

			loop1:
				;

			} finally { DebugExitSubRule(1); }

			DebugLocation(403, 43);
			mAPOSTRAPHE(); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("STRING_LITERAL", 55);
			LeaveRule("STRING_LITERAL", 55);
			LeaveRule_STRING_LITERAL();
		}
	}
	// $ANTLR end "STRING_LITERAL"

	partial void EnterRule_ID();
	partial void LeaveRule_ID();

	// $ANTLR start "ID"
	[GrammarRule("ID")]
	private void mID()
	{
		EnterRule_ID();
		EnterRule("ID", 56);
		TraceIn("ID", 56);
		try
		{
			int _type = ID;
			int _channel = DefaultTokenChannel;
			// Scrape.g:404:4: ( ( VARIABLE )+ )
			DebugEnterAlt(1);
			// Scrape.g:404:6: ( VARIABLE )+
			{
			DebugLocation(404, 6);
			// Scrape.g:404:6: ( VARIABLE )+
			int cnt2=0;
			try { DebugEnterSubRule(2);
			while (true)
			{
				int alt2=2;
				try { DebugEnterDecision(2, false);
				int LA2_1 = input.LA(1);

				if (((LA2_1>='@' && LA2_1<='Z')||(LA2_1>='a' && LA2_1<='z')))
				{
					alt2 = 1;
				}


				} finally { DebugExitDecision(2); }
				switch (alt2)
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:
					{
					DebugLocation(404, 6);
					input.Consume();


					}
					break;

				default:
					if (cnt2 >= 1)
						goto loop2;

					EarlyExitException eee2 = new EarlyExitException( 2, input );
					DebugRecognitionException(eee2);
					throw eee2;
				}
				cnt2++;
			}
			loop2:
				;

			} finally { DebugExitSubRule(2); }


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("ID", 56);
			LeaveRule("ID", 56);
			LeaveRule_ID();
		}
	}
	// $ANTLR end "ID"

	partial void EnterRule_ASTERISK();
	partial void LeaveRule_ASTERISK();

	// $ANTLR start "ASTERISK"
	[GrammarRule("ASTERISK")]
	private void mASTERISK()
	{
		EnterRule_ASTERISK();
		EnterRule("ASTERISK", 57);
		TraceIn("ASTERISK", 57);
		try
		{
			int _type = ASTERISK;
			int _channel = DefaultTokenChannel;
			// Scrape.g:405:10: ( '*' )
			DebugEnterAlt(1);
			// Scrape.g:405:12: '*'
			{
			DebugLocation(405, 12);
			Match('*'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("ASTERISK", 57);
			LeaveRule("ASTERISK", 57);
			LeaveRule_ASTERISK();
		}
	}
	// $ANTLR end "ASTERISK"

	partial void EnterRule_DOT();
	partial void LeaveRule_DOT();

	// $ANTLR start "DOT"
	[GrammarRule("DOT")]
	private void mDOT()
	{
		EnterRule_DOT();
		EnterRule("DOT", 58);
		TraceIn("DOT", 58);
		try
		{
			int _type = DOT;
			int _channel = DefaultTokenChannel;
			// Scrape.g:406:5: ( '.' )
			DebugEnterAlt(1);
			// Scrape.g:406:7: '.'
			{
			DebugLocation(406, 7);
			Match('.'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("DOT", 58);
			LeaveRule("DOT", 58);
			LeaveRule_DOT();
		}
	}
	// $ANTLR end "DOT"

	partial void EnterRule_OPENPAREN();
	partial void LeaveRule_OPENPAREN();

	// $ANTLR start "OPENPAREN"
	[GrammarRule("OPENPAREN")]
	private void mOPENPAREN()
	{
		EnterRule_OPENPAREN();
		EnterRule("OPENPAREN", 59);
		TraceIn("OPENPAREN", 59);
		try
		{
			int _type = OPENPAREN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:407:11: ( '(' )
			DebugEnterAlt(1);
			// Scrape.g:407:13: '('
			{
			DebugLocation(407, 13);
			Match('('); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("OPENPAREN", 59);
			LeaveRule("OPENPAREN", 59);
			LeaveRule_OPENPAREN();
		}
	}
	// $ANTLR end "OPENPAREN"

	partial void EnterRule_CLOSEPAREN();
	partial void LeaveRule_CLOSEPAREN();

	// $ANTLR start "CLOSEPAREN"
	[GrammarRule("CLOSEPAREN")]
	private void mCLOSEPAREN()
	{
		EnterRule_CLOSEPAREN();
		EnterRule("CLOSEPAREN", 60);
		TraceIn("CLOSEPAREN", 60);
		try
		{
			int _type = CLOSEPAREN;
			int _channel = DefaultTokenChannel;
			// Scrape.g:408:12: ( ')' )
			DebugEnterAlt(1);
			// Scrape.g:408:14: ')'
			{
			DebugLocation(408, 14);
			Match(')'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("CLOSEPAREN", 60);
			LeaveRule("CLOSEPAREN", 60);
			LeaveRule_CLOSEPAREN();
		}
	}
	// $ANTLR end "CLOSEPAREN"

	partial void EnterRule_OPENBRACE();
	partial void LeaveRule_OPENBRACE();

	// $ANTLR start "OPENBRACE"
	[GrammarRule("OPENBRACE")]
	private void mOPENBRACE()
	{
		EnterRule_OPENBRACE();
		EnterRule("OPENBRACE", 61);
		TraceIn("OPENBRACE", 61);
		try
		{
			int _type = OPENBRACE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:409:11: ( '{' )
			DebugEnterAlt(1);
			// Scrape.g:409:13: '{'
			{
			DebugLocation(409, 13);
			Match('{'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("OPENBRACE", 61);
			LeaveRule("OPENBRACE", 61);
			LeaveRule_OPENBRACE();
		}
	}
	// $ANTLR end "OPENBRACE"

	partial void EnterRule_CLOSEBRACE();
	partial void LeaveRule_CLOSEBRACE();

	// $ANTLR start "CLOSEBRACE"
	[GrammarRule("CLOSEBRACE")]
	private void mCLOSEBRACE()
	{
		EnterRule_CLOSEBRACE();
		EnterRule("CLOSEBRACE", 62);
		TraceIn("CLOSEBRACE", 62);
		try
		{
			int _type = CLOSEBRACE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:410:12: ( '}' )
			DebugEnterAlt(1);
			// Scrape.g:410:14: '}'
			{
			DebugLocation(410, 14);
			Match('}'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("CLOSEBRACE", 62);
			LeaveRule("CLOSEBRACE", 62);
			LeaveRule_CLOSEBRACE();
		}
	}
	// $ANTLR end "CLOSEBRACE"

	partial void EnterRule_PLUS();
	partial void LeaveRule_PLUS();

	// $ANTLR start "PLUS"
	[GrammarRule("PLUS")]
	private void mPLUS()
	{
		EnterRule_PLUS();
		EnterRule("PLUS", 63);
		TraceIn("PLUS", 63);
		try
		{
			int _type = PLUS;
			int _channel = DefaultTokenChannel;
			// Scrape.g:411:6: ( '+' )
			DebugEnterAlt(1);
			// Scrape.g:411:8: '+'
			{
			DebugLocation(411, 8);
			Match('+'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("PLUS", 63);
			LeaveRule("PLUS", 63);
			LeaveRule_PLUS();
		}
	}
	// $ANTLR end "PLUS"

	partial void EnterRule_MINIS();
	partial void LeaveRule_MINIS();

	// $ANTLR start "MINIS"
	[GrammarRule("MINIS")]
	private void mMINIS()
	{
		EnterRule_MINIS();
		EnterRule("MINIS", 64);
		TraceIn("MINIS", 64);
		try
		{
			int _type = MINIS;
			int _channel = DefaultTokenChannel;
			// Scrape.g:412:8: ( '-' )
			DebugEnterAlt(1);
			// Scrape.g:412:10: '-'
			{
			DebugLocation(412, 10);
			Match('-'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("MINIS", 64);
			LeaveRule("MINIS", 64);
			LeaveRule_MINIS();
		}
	}
	// $ANTLR end "MINIS"

	partial void EnterRule_DIV();
	partial void LeaveRule_DIV();

	// $ANTLR start "DIV"
	[GrammarRule("DIV")]
	private void mDIV()
	{
		EnterRule_DIV();
		EnterRule("DIV", 65);
		TraceIn("DIV", 65);
		try
		{
			int _type = DIV;
			int _channel = DefaultTokenChannel;
			// Scrape.g:413:6: ( '/' )
			DebugEnterAlt(1);
			// Scrape.g:413:8: '/'
			{
			DebugLocation(413, 8);
			Match('/'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("DIV", 65);
			LeaveRule("DIV", 65);
			LeaveRule_DIV();
		}
	}
	// $ANTLR end "DIV"

	partial void EnterRule_COMMA();
	partial void LeaveRule_COMMA();

	// $ANTLR start "COMMA"
	[GrammarRule("COMMA")]
	private void mCOMMA()
	{
		EnterRule_COMMA();
		EnterRule("COMMA", 66);
		TraceIn("COMMA", 66);
		try
		{
			int _type = COMMA;
			int _channel = DefaultTokenChannel;
			// Scrape.g:414:7: ( ',' )
			DebugEnterAlt(1);
			// Scrape.g:414:9: ','
			{
			DebugLocation(414, 9);
			Match(','); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("COMMA", 66);
			LeaveRule("COMMA", 66);
			LeaveRule_COMMA();
		}
	}
	// $ANTLR end "COMMA"

	partial void EnterRule_QUOTE();
	partial void LeaveRule_QUOTE();

	// $ANTLR start "QUOTE"
	[GrammarRule("QUOTE")]
	private void mQUOTE()
	{
		EnterRule_QUOTE();
		EnterRule("QUOTE", 67);
		TraceIn("QUOTE", 67);
		try
		{
			int _type = QUOTE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:415:7: ( '\"' )
			DebugEnterAlt(1);
			// Scrape.g:415:9: '\"'
			{
			DebugLocation(415, 9);
			Match('\"'); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("QUOTE", 67);
			LeaveRule("QUOTE", 67);
			LeaveRule_QUOTE();
		}
	}
	// $ANTLR end "QUOTE"

	partial void EnterRule_APOSTRAPHE();
	partial void LeaveRule_APOSTRAPHE();

	// $ANTLR start "APOSTRAPHE"
	[GrammarRule("APOSTRAPHE")]
	private void mAPOSTRAPHE()
	{
		EnterRule_APOSTRAPHE();
		EnterRule("APOSTRAPHE", 68);
		TraceIn("APOSTRAPHE", 68);
		try
		{
			int _type = APOSTRAPHE;
			int _channel = DefaultTokenChannel;
			// Scrape.g:416:12: ( '\\'' )
			DebugEnterAlt(1);
			// Scrape.g:416:14: '\\''
			{
			DebugLocation(416, 14);
			Match('\''); 

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("APOSTRAPHE", 68);
			LeaveRule("APOSTRAPHE", 68);
			LeaveRule_APOSTRAPHE();
		}
	}
	// $ANTLR end "APOSTRAPHE"

	partial void EnterRule_INT();
	partial void LeaveRule_INT();

	// $ANTLR start "INT"
	[GrammarRule("INT")]
	private void mINT()
	{
		EnterRule_INT();
		EnterRule("INT", 69);
		TraceIn("INT", 69);
		try
		{
			int _type = INT;
			int _channel = DefaultTokenChannel;
			// Scrape.g:417:5: ( ( DIGIT )+ )
			DebugEnterAlt(1);
			// Scrape.g:417:7: ( DIGIT )+
			{
			DebugLocation(417, 7);
			// Scrape.g:417:7: ( DIGIT )+
			int cnt3=0;
			try { DebugEnterSubRule(3);
			while (true)
			{
				int alt3=2;
				try { DebugEnterDecision(3, false);
				int LA3_1 = input.LA(1);

				if (((LA3_1>='0' && LA3_1<='9')))
				{
					alt3 = 1;
				}


				} finally { DebugExitDecision(3); }
				switch (alt3)
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:
					{
					DebugLocation(417, 7);
					input.Consume();


					}
					break;

				default:
					if (cnt3 >= 1)
						goto loop3;

					EarlyExitException eee3 = new EarlyExitException( 3, input );
					DebugRecognitionException(eee3);
					throw eee3;
				}
				cnt3++;
			}
			loop3:
				;

			} finally { DebugExitSubRule(3); }


			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("INT", 69);
			LeaveRule("INT", 69);
			LeaveRule_INT();
		}
	}
	// $ANTLR end "INT"

	partial void EnterRule_NEWLINE();
	partial void LeaveRule_NEWLINE();

	// $ANTLR start "NEWLINE"
	[GrammarRule("NEWLINE")]
	private void mNEWLINE()
	{
		EnterRule_NEWLINE();
		EnterRule("NEWLINE", 70);
		TraceIn("NEWLINE", 70);
		try
		{
			// Scrape.g:418:18: ( ( '\\n' | '\\r' ) )
			DebugEnterAlt(1);
			// Scrape.g:
			{
			DebugLocation(418, 18);
			if (input.LA(1)=='\n'||input.LA(1)=='\r')
			{
				input.Consume();
			}
			else
			{
				MismatchedSetException mse = new MismatchedSetException(null,input);
				DebugRecognitionException(mse);
				Recover(mse);
				throw mse;
			}


			}

		}
		finally
		{
			TraceOut("NEWLINE", 70);
			LeaveRule("NEWLINE", 70);
			LeaveRule_NEWLINE();
		}
	}
	// $ANTLR end "NEWLINE"

	partial void EnterRule_DIGIT();
	partial void LeaveRule_DIGIT();

	// $ANTLR start "DIGIT"
	[GrammarRule("DIGIT")]
	private void mDIGIT()
	{
		EnterRule_DIGIT();
		EnterRule("DIGIT", 71);
		TraceIn("DIGIT", 71);
		try
		{
			// Scrape.g:419:15: ( '0' .. '9' )
			DebugEnterAlt(1);
			// Scrape.g:
			{
			DebugLocation(419, 15);
			if ((input.LA(1)>='0' && input.LA(1)<='9'))
			{
				input.Consume();
			}
			else
			{
				MismatchedSetException mse = new MismatchedSetException(null,input);
				DebugRecognitionException(mse);
				Recover(mse);
				throw mse;
			}


			}

		}
		finally
		{
			TraceOut("DIGIT", 71);
			LeaveRule("DIGIT", 71);
			LeaveRule_DIGIT();
		}
	}
	// $ANTLR end "DIGIT"

	partial void EnterRule_VARIABLE();
	partial void LeaveRule_VARIABLE();

	// $ANTLR start "VARIABLE"
	[GrammarRule("VARIABLE")]
	private void mVARIABLE()
	{
		EnterRule_VARIABLE();
		EnterRule("VARIABLE", 72);
		TraceIn("VARIABLE", 72);
		try
		{
			// Scrape.g:420:18: ( ( '@' | LETTER ) )
			DebugEnterAlt(1);
			// Scrape.g:
			{
			DebugLocation(420, 18);
			if ((input.LA(1)>='@' && input.LA(1)<='Z')||(input.LA(1)>='a' && input.LA(1)<='z'))
			{
				input.Consume();
			}
			else
			{
				MismatchedSetException mse = new MismatchedSetException(null,input);
				DebugRecognitionException(mse);
				Recover(mse);
				throw mse;
			}


			}

		}
		finally
		{
			TraceOut("VARIABLE", 72);
			LeaveRule("VARIABLE", 72);
			LeaveRule_VARIABLE();
		}
	}
	// $ANTLR end "VARIABLE"

	partial void EnterRule_LETTER();
	partial void LeaveRule_LETTER();

	// $ANTLR start "LETTER"
	[GrammarRule("LETTER")]
	private void mLETTER()
	{
		EnterRule_LETTER();
		EnterRule("LETTER", 73);
		TraceIn("LETTER", 73);
		try
		{
			// Scrape.g:421:17: ( ( 'a' .. 'z' | 'A' .. 'Z' ) )
			DebugEnterAlt(1);
			// Scrape.g:
			{
			DebugLocation(421, 17);
			if ((input.LA(1)>='A' && input.LA(1)<='Z')||(input.LA(1)>='a' && input.LA(1)<='z'))
			{
				input.Consume();
			}
			else
			{
				MismatchedSetException mse = new MismatchedSetException(null,input);
				DebugRecognitionException(mse);
				Recover(mse);
				throw mse;
			}


			}

		}
		finally
		{
			TraceOut("LETTER", 73);
			LeaveRule("LETTER", 73);
			LeaveRule_LETTER();
		}
	}
	// $ANTLR end "LETTER"

	partial void EnterRule_OTHERCHARS();
	partial void LeaveRule_OTHERCHARS();

	// $ANTLR start "OTHERCHARS"
	[GrammarRule("OTHERCHARS")]
	private void mOTHERCHARS()
	{
		EnterRule_OTHERCHARS();
		EnterRule("OTHERCHARS", 74);
		TraceIn("OTHERCHARS", 74);
		try
		{
			// Scrape.g:422:21: ( ( '.' | '|' | '-' | '&' | ',' | '\\\\' | ':' ) )
			DebugEnterAlt(1);
			// Scrape.g:
			{
			DebugLocation(422, 21);
			if (input.LA(1)=='&'||(input.LA(1)>=',' && input.LA(1)<='.')||input.LA(1)==':'||input.LA(1)=='\\'||input.LA(1)=='|')
			{
				input.Consume();
			}
			else
			{
				MismatchedSetException mse = new MismatchedSetException(null,input);
				DebugRecognitionException(mse);
				Recover(mse);
				throw mse;
			}


			}

		}
		finally
		{
			TraceOut("OTHERCHARS", 74);
			LeaveRule("OTHERCHARS", 74);
			LeaveRule_OTHERCHARS();
		}
	}
	// $ANTLR end "OTHERCHARS"

	partial void EnterRule_WS();
	partial void LeaveRule_WS();

	// $ANTLR start "WS"
	[GrammarRule("WS")]
	private void mWS()
	{
		EnterRule_WS();
		EnterRule("WS", 75);
		TraceIn("WS", 75);
		try
		{
			int _type = WS;
			int _channel = DefaultTokenChannel;
			// Scrape.g:423:4: ( ( ' ' | '\\t' | NEWLINE )+ )
			DebugEnterAlt(1);
			// Scrape.g:423:7: ( ' ' | '\\t' | NEWLINE )+
			{
			DebugLocation(423, 7);
			// Scrape.g:423:7: ( ' ' | '\\t' | NEWLINE )+
			int cnt4=0;
			try { DebugEnterSubRule(4);
			while (true)
			{
				int alt4=2;
				try { DebugEnterDecision(4, false);
				int LA4_1 = input.LA(1);

				if (((LA4_1>='\t' && LA4_1<='\n')||LA4_1=='\r'||LA4_1==' '))
				{
					alt4 = 1;
				}


				} finally { DebugExitDecision(4); }
				switch (alt4)
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:
					{
					DebugLocation(423, 7);
					input.Consume();


					}
					break;

				default:
					if (cnt4 >= 1)
						goto loop4;

					EarlyExitException eee4 = new EarlyExitException( 4, input );
					DebugRecognitionException(eee4);
					throw eee4;
				}
				cnt4++;
			}
			loop4:
				;

			} finally { DebugExitSubRule(4); }

			DebugLocation(423, 27);
			_channel = Hidden;

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("WS", 75);
			LeaveRule("WS", 75);
			LeaveRule_WS();
		}
	}
	// $ANTLR end "WS"

	partial void EnterRule_COMMENT();
	partial void LeaveRule_COMMENT();

	// $ANTLR start "COMMENT"
	[GrammarRule("COMMENT")]
	private void mCOMMENT()
	{
		EnterRule_COMMENT();
		EnterRule("COMMENT", 76);
		TraceIn("COMMENT", 76);
		try
		{
			int _type = COMMENT;
			int _channel = DefaultTokenChannel;
			// Scrape.g:424:9: ( '/*' ( . )* '*/' )
			DebugEnterAlt(1);
			// Scrape.g:424:11: '/*' ( . )* '*/'
			{
			DebugLocation(424, 11);
			Match("/*"); 

			DebugLocation(424, 16);
			// Scrape.g:424:16: ( . )*
			try { DebugEnterSubRule(5);
			while (true)
			{
				int alt5=2;
				try { DebugEnterDecision(5, false);
				int LA5_1 = input.LA(1);

				if ((LA5_1=='*'))
				{
					int LA5_2 = input.LA(2);

					if ((LA5_2=='/'))
					{
						alt5 = 2;
					}
					else if (((LA5_2>='\u0000' && LA5_2<='.')||(LA5_2>='0' && LA5_2<='\uFFFF')))
					{
						alt5 = 1;
					}


				}
				else if (((LA5_1>='\u0000' && LA5_1<=')')||(LA5_1>='+' && LA5_1<='\uFFFF')))
				{
					alt5 = 1;
				}


				} finally { DebugExitDecision(5); }
				switch ( alt5 )
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:424:16: .
					{
					DebugLocation(424, 16);
					MatchAny(); 

					}
					break;

				default:
					goto loop5;
				}
			}

			loop5:
				;

			} finally { DebugExitSubRule(5); }

			DebugLocation(424, 19);
			Match("*/"); 

			DebugLocation(424, 24);
			_channel = Hidden;

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("COMMENT", 76);
			LeaveRule("COMMENT", 76);
			LeaveRule_COMMENT();
		}
	}
	// $ANTLR end "COMMENT"

	partial void EnterRule_LINE_COMMENT_SLASH();
	partial void LeaveRule_LINE_COMMENT_SLASH();

	// $ANTLR start "LINE_COMMENT_SLASH"
	[GrammarRule("LINE_COMMENT_SLASH")]
	private void mLINE_COMMENT_SLASH()
	{
		EnterRule_LINE_COMMENT_SLASH();
		EnterRule("LINE_COMMENT_SLASH", 77);
		TraceIn("LINE_COMMENT_SLASH", 77);
		try
		{
			int _type = LINE_COMMENT_SLASH;
			int _channel = DefaultTokenChannel;
			// Scrape.g:425:20: ( '//' (~ NEWLINE )* )
			DebugEnterAlt(1);
			// Scrape.g:425:22: '//' (~ NEWLINE )*
			{
			DebugLocation(425, 22);
			Match("//"); 

			DebugLocation(425, 27);
			// Scrape.g:425:27: (~ NEWLINE )*
			try { DebugEnterSubRule(6);
			while (true)
			{
				int alt6=2;
				try { DebugEnterDecision(6, false);
				int LA6_1 = input.LA(1);

				if (((LA6_1>='\u0000' && LA6_1<='\t')||(LA6_1>='\u000B' && LA6_1<='\f')||(LA6_1>='\u000E' && LA6_1<='\uFFFF')))
				{
					alt6 = 1;
				}


				} finally { DebugExitDecision(6); }
				switch ( alt6 )
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:
					{
					DebugLocation(425, 27);
					input.Consume();


					}
					break;

				default:
					goto loop6;
				}
			}

			loop6:
				;

			} finally { DebugExitSubRule(6); }

			DebugLocation(425, 37);
			_channel = Hidden;

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("LINE_COMMENT_SLASH", 77);
			LeaveRule("LINE_COMMENT_SLASH", 77);
			LeaveRule_LINE_COMMENT_SLASH();
		}
	}
	// $ANTLR end "LINE_COMMENT_SLASH"

	partial void EnterRule_LINE_COMMENT_DASH();
	partial void LeaveRule_LINE_COMMENT_DASH();

	// $ANTLR start "LINE_COMMENT_DASH"
	[GrammarRule("LINE_COMMENT_DASH")]
	private void mLINE_COMMENT_DASH()
	{
		EnterRule_LINE_COMMENT_DASH();
		EnterRule("LINE_COMMENT_DASH", 78);
		TraceIn("LINE_COMMENT_DASH", 78);
		try
		{
			int _type = LINE_COMMENT_DASH;
			int _channel = DefaultTokenChannel;
			// Scrape.g:426:19: ( '--' (~ NEWLINE )* )
			DebugEnterAlt(1);
			// Scrape.g:426:21: '--' (~ NEWLINE )*
			{
			DebugLocation(426, 21);
			Match("--"); 

			DebugLocation(426, 26);
			// Scrape.g:426:26: (~ NEWLINE )*
			try { DebugEnterSubRule(7);
			while (true)
			{
				int alt7=2;
				try { DebugEnterDecision(7, false);
				int LA7_1 = input.LA(1);

				if (((LA7_1>='\u0000' && LA7_1<='\t')||(LA7_1>='\u000B' && LA7_1<='\f')||(LA7_1>='\u000E' && LA7_1<='\uFFFF')))
				{
					alt7 = 1;
				}


				} finally { DebugExitDecision(7); }
				switch ( alt7 )
				{
				case 1:
					DebugEnterAlt(1);
					// Scrape.g:
					{
					DebugLocation(426, 26);
					input.Consume();


					}
					break;

				default:
					goto loop7;
				}
			}

			loop7:
				;

			} finally { DebugExitSubRule(7); }

			DebugLocation(426, 36);
			_channel = Hidden;

			}

			state.type = _type;
			state.channel = _channel;
		}
		finally
		{
			TraceOut("LINE_COMMENT_DASH", 78);
			LeaveRule("LINE_COMMENT_DASH", 78);
			LeaveRule_LINE_COMMENT_DASH();
		}
	}
	// $ANTLR end "LINE_COMMENT_DASH"

	public override void mTokens()
	{
		// Scrape.g:1:8: ( T__110 | T__111 | PROCEDURE | EXEC | CASE | WHEN | THEN | END | ELSE | AND | OR | EQUALS | LESSTHAN | LESSTHANEQUAL | GREATERTHAN | GREATERTHANEQUAL | NOTEQUAL | INSERT_INTO | INSERT_OVERWRITE | INSERT_DIRECTORY | TRUNCATE | EACH | IN | SELECT | FROM | INNER_JOIN | JOIN | NODES | WHERE | PICK | TAKE | MATCH | REPLACE | ATTRIBUTE | TEXT | HTML | DOWNLOAD_IMAGE | DOWNLOAD_PAGE | IDENTITY | EXPAND | TO | PROXIES | TEST | NULL | CREATE | FILE | BUFFER | WITH | STRING | INTEGER | FLOAT | FIELD_TERMINATOR | ROW_TERMINATOR | LOCATION | STRING_LITERAL | ID | ASTERISK | DOT | OPENPAREN | CLOSEPAREN | OPENBRACE | CLOSEBRACE | PLUS | MINIS | DIV | COMMA | QUOTE | APOSTRAPHE | INT | WS | COMMENT | LINE_COMMENT_SLASH | LINE_COMMENT_DASH )
		int alt8=73;
		try { DebugEnterDecision(8, false);
		try
		{
			alt8 = dfa8.Predict(input);
		}
		catch (NoViableAltException nvae)
		{
			DebugRecognitionException(nvae);
			throw;
		}
		} finally { DebugExitDecision(8); }
		switch (alt8)
		{
		case 1:
			DebugEnterAlt(1);
			// Scrape.g:1:10: T__110
			{
			DebugLocation(1, 10);
			mT__110(); 

			}
			break;
		case 2:
			DebugEnterAlt(2);
			// Scrape.g:1:17: T__111
			{
			DebugLocation(1, 17);
			mT__111(); 

			}
			break;
		case 3:
			DebugEnterAlt(3);
			// Scrape.g:1:24: PROCEDURE
			{
			DebugLocation(1, 24);
			mPROCEDURE(); 

			}
			break;
		case 4:
			DebugEnterAlt(4);
			// Scrape.g:1:34: EXEC
			{
			DebugLocation(1, 34);
			mEXEC(); 

			}
			break;
		case 5:
			DebugEnterAlt(5);
			// Scrape.g:1:39: CASE
			{
			DebugLocation(1, 39);
			mCASE(); 

			}
			break;
		case 6:
			DebugEnterAlt(6);
			// Scrape.g:1:44: WHEN
			{
			DebugLocation(1, 44);
			mWHEN(); 

			}
			break;
		case 7:
			DebugEnterAlt(7);
			// Scrape.g:1:49: THEN
			{
			DebugLocation(1, 49);
			mTHEN(); 

			}
			break;
		case 8:
			DebugEnterAlt(8);
			// Scrape.g:1:54: END
			{
			DebugLocation(1, 54);
			mEND(); 

			}
			break;
		case 9:
			DebugEnterAlt(9);
			// Scrape.g:1:58: ELSE
			{
			DebugLocation(1, 58);
			mELSE(); 

			}
			break;
		case 10:
			DebugEnterAlt(10);
			// Scrape.g:1:63: AND
			{
			DebugLocation(1, 63);
			mAND(); 

			}
			break;
		case 11:
			DebugEnterAlt(11);
			// Scrape.g:1:67: OR
			{
			DebugLocation(1, 67);
			mOR(); 

			}
			break;
		case 12:
			DebugEnterAlt(12);
			// Scrape.g:1:70: EQUALS
			{
			DebugLocation(1, 70);
			mEQUALS(); 

			}
			break;
		case 13:
			DebugEnterAlt(13);
			// Scrape.g:1:77: LESSTHAN
			{
			DebugLocation(1, 77);
			mLESSTHAN(); 

			}
			break;
		case 14:
			DebugEnterAlt(14);
			// Scrape.g:1:86: LESSTHANEQUAL
			{
			DebugLocation(1, 86);
			mLESSTHANEQUAL(); 

			}
			break;
		case 15:
			DebugEnterAlt(15);
			// Scrape.g:1:100: GREATERTHAN
			{
			DebugLocation(1, 100);
			mGREATERTHAN(); 

			}
			break;
		case 16:
			DebugEnterAlt(16);
			// Scrape.g:1:112: GREATERTHANEQUAL
			{
			DebugLocation(1, 112);
			mGREATERTHANEQUAL(); 

			}
			break;
		case 17:
			DebugEnterAlt(17);
			// Scrape.g:1:129: NOTEQUAL
			{
			DebugLocation(1, 129);
			mNOTEQUAL(); 

			}
			break;
		case 18:
			DebugEnterAlt(18);
			// Scrape.g:1:138: INSERT_INTO
			{
			DebugLocation(1, 138);
			mINSERT_INTO(); 

			}
			break;
		case 19:
			DebugEnterAlt(19);
			// Scrape.g:1:150: INSERT_OVERWRITE
			{
			DebugLocation(1, 150);
			mINSERT_OVERWRITE(); 

			}
			break;
		case 20:
			DebugEnterAlt(20);
			// Scrape.g:1:167: INSERT_DIRECTORY
			{
			DebugLocation(1, 167);
			mINSERT_DIRECTORY(); 

			}
			break;
		case 21:
			DebugEnterAlt(21);
			// Scrape.g:1:184: TRUNCATE
			{
			DebugLocation(1, 184);
			mTRUNCATE(); 

			}
			break;
		case 22:
			DebugEnterAlt(22);
			// Scrape.g:1:193: EACH
			{
			DebugLocation(1, 193);
			mEACH(); 

			}
			break;
		case 23:
			DebugEnterAlt(23);
			// Scrape.g:1:198: IN
			{
			DebugLocation(1, 198);
			mIN(); 

			}
			break;
		case 24:
			DebugEnterAlt(24);
			// Scrape.g:1:201: SELECT
			{
			DebugLocation(1, 201);
			mSELECT(); 

			}
			break;
		case 25:
			DebugEnterAlt(25);
			// Scrape.g:1:208: FROM
			{
			DebugLocation(1, 208);
			mFROM(); 

			}
			break;
		case 26:
			DebugEnterAlt(26);
			// Scrape.g:1:213: INNER_JOIN
			{
			DebugLocation(1, 213);
			mINNER_JOIN(); 

			}
			break;
		case 27:
			DebugEnterAlt(27);
			// Scrape.g:1:224: JOIN
			{
			DebugLocation(1, 224);
			mJOIN(); 

			}
			break;
		case 28:
			DebugEnterAlt(28);
			// Scrape.g:1:229: NODES
			{
			DebugLocation(1, 229);
			mNODES(); 

			}
			break;
		case 29:
			DebugEnterAlt(29);
			// Scrape.g:1:235: WHERE
			{
			DebugLocation(1, 235);
			mWHERE(); 

			}
			break;
		case 30:
			DebugEnterAlt(30);
			// Scrape.g:1:241: PICK
			{
			DebugLocation(1, 241);
			mPICK(); 

			}
			break;
		case 31:
			DebugEnterAlt(31);
			// Scrape.g:1:246: TAKE
			{
			DebugLocation(1, 246);
			mTAKE(); 

			}
			break;
		case 32:
			DebugEnterAlt(32);
			// Scrape.g:1:251: MATCH
			{
			DebugLocation(1, 251);
			mMATCH(); 

			}
			break;
		case 33:
			DebugEnterAlt(33);
			// Scrape.g:1:257: REPLACE
			{
			DebugLocation(1, 257);
			mREPLACE(); 

			}
			break;
		case 34:
			DebugEnterAlt(34);
			// Scrape.g:1:265: ATTRIBUTE
			{
			DebugLocation(1, 265);
			mATTRIBUTE(); 

			}
			break;
		case 35:
			DebugEnterAlt(35);
			// Scrape.g:1:275: TEXT
			{
			DebugLocation(1, 275);
			mTEXT(); 

			}
			break;
		case 36:
			DebugEnterAlt(36);
			// Scrape.g:1:280: HTML
			{
			DebugLocation(1, 280);
			mHTML(); 

			}
			break;
		case 37:
			DebugEnterAlt(37);
			// Scrape.g:1:285: DOWNLOAD_IMAGE
			{
			DebugLocation(1, 285);
			mDOWNLOAD_IMAGE(); 

			}
			break;
		case 38:
			DebugEnterAlt(38);
			// Scrape.g:1:300: DOWNLOAD_PAGE
			{
			DebugLocation(1, 300);
			mDOWNLOAD_PAGE(); 

			}
			break;
		case 39:
			DebugEnterAlt(39);
			// Scrape.g:1:314: IDENTITY
			{
			DebugLocation(1, 314);
			mIDENTITY(); 

			}
			break;
		case 40:
			DebugEnterAlt(40);
			// Scrape.g:1:323: EXPAND
			{
			DebugLocation(1, 323);
			mEXPAND(); 

			}
			break;
		case 41:
			DebugEnterAlt(41);
			// Scrape.g:1:330: TO
			{
			DebugLocation(1, 330);
			mTO(); 

			}
			break;
		case 42:
			DebugEnterAlt(42);
			// Scrape.g:1:333: PROXIES
			{
			DebugLocation(1, 333);
			mPROXIES(); 

			}
			break;
		case 43:
			DebugEnterAlt(43);
			// Scrape.g:1:341: TEST
			{
			DebugLocation(1, 341);
			mTEST(); 

			}
			break;
		case 44:
			DebugEnterAlt(44);
			// Scrape.g:1:346: NULL
			{
			DebugLocation(1, 346);
			mNULL(); 

			}
			break;
		case 45:
			DebugEnterAlt(45);
			// Scrape.g:1:351: CREATE
			{
			DebugLocation(1, 351);
			mCREATE(); 

			}
			break;
		case 46:
			DebugEnterAlt(46);
			// Scrape.g:1:358: FILE
			{
			DebugLocation(1, 358);
			mFILE(); 

			}
			break;
		case 47:
			DebugEnterAlt(47);
			// Scrape.g:1:363: BUFFER
			{
			DebugLocation(1, 363);
			mBUFFER(); 

			}
			break;
		case 48:
			DebugEnterAlt(48);
			// Scrape.g:1:370: WITH
			{
			DebugLocation(1, 370);
			mWITH(); 

			}
			break;
		case 49:
			DebugEnterAlt(49);
			// Scrape.g:1:375: STRING
			{
			DebugLocation(1, 375);
			mSTRING(); 

			}
			break;
		case 50:
			DebugEnterAlt(50);
			// Scrape.g:1:382: INTEGER
			{
			DebugLocation(1, 382);
			mINTEGER(); 

			}
			break;
		case 51:
			DebugEnterAlt(51);
			// Scrape.g:1:390: FLOAT
			{
			DebugLocation(1, 390);
			mFLOAT(); 

			}
			break;
		case 52:
			DebugEnterAlt(52);
			// Scrape.g:1:396: FIELD_TERMINATOR
			{
			DebugLocation(1, 396);
			mFIELD_TERMINATOR(); 

			}
			break;
		case 53:
			DebugEnterAlt(53);
			// Scrape.g:1:413: ROW_TERMINATOR
			{
			DebugLocation(1, 413);
			mROW_TERMINATOR(); 

			}
			break;
		case 54:
			DebugEnterAlt(54);
			// Scrape.g:1:428: LOCATION
			{
			DebugLocation(1, 428);
			mLOCATION(); 

			}
			break;
		case 55:
			DebugEnterAlt(55);
			// Scrape.g:1:437: STRING_LITERAL
			{
			DebugLocation(1, 437);
			mSTRING_LITERAL(); 

			}
			break;
		case 56:
			DebugEnterAlt(56);
			// Scrape.g:1:452: ID
			{
			DebugLocation(1, 452);
			mID(); 

			}
			break;
		case 57:
			DebugEnterAlt(57);
			// Scrape.g:1:455: ASTERISK
			{
			DebugLocation(1, 455);
			mASTERISK(); 

			}
			break;
		case 58:
			DebugEnterAlt(58);
			// Scrape.g:1:464: DOT
			{
			DebugLocation(1, 464);
			mDOT(); 

			}
			break;
		case 59:
			DebugEnterAlt(59);
			// Scrape.g:1:468: OPENPAREN
			{
			DebugLocation(1, 468);
			mOPENPAREN(); 

			}
			break;
		case 60:
			DebugEnterAlt(60);
			// Scrape.g:1:478: CLOSEPAREN
			{
			DebugLocation(1, 478);
			mCLOSEPAREN(); 

			}
			break;
		case 61:
			DebugEnterAlt(61);
			// Scrape.g:1:489: OPENBRACE
			{
			DebugLocation(1, 489);
			mOPENBRACE(); 

			}
			break;
		case 62:
			DebugEnterAlt(62);
			// Scrape.g:1:499: CLOSEBRACE
			{
			DebugLocation(1, 499);
			mCLOSEBRACE(); 

			}
			break;
		case 63:
			DebugEnterAlt(63);
			// Scrape.g:1:510: PLUS
			{
			DebugLocation(1, 510);
			mPLUS(); 

			}
			break;
		case 64:
			DebugEnterAlt(64);
			// Scrape.g:1:515: MINIS
			{
			DebugLocation(1, 515);
			mMINIS(); 

			}
			break;
		case 65:
			DebugEnterAlt(65);
			// Scrape.g:1:521: DIV
			{
			DebugLocation(1, 521);
			mDIV(); 

			}
			break;
		case 66:
			DebugEnterAlt(66);
			// Scrape.g:1:525: COMMA
			{
			DebugLocation(1, 525);
			mCOMMA(); 

			}
			break;
		case 67:
			DebugEnterAlt(67);
			// Scrape.g:1:531: QUOTE
			{
			DebugLocation(1, 531);
			mQUOTE(); 

			}
			break;
		case 68:
			DebugEnterAlt(68);
			// Scrape.g:1:537: APOSTRAPHE
			{
			DebugLocation(1, 537);
			mAPOSTRAPHE(); 

			}
			break;
		case 69:
			DebugEnterAlt(69);
			// Scrape.g:1:548: INT
			{
			DebugLocation(1, 548);
			mINT(); 

			}
			break;
		case 70:
			DebugEnterAlt(70);
			// Scrape.g:1:552: WS
			{
			DebugLocation(1, 552);
			mWS(); 

			}
			break;
		case 71:
			DebugEnterAlt(71);
			// Scrape.g:1:555: COMMENT
			{
			DebugLocation(1, 555);
			mCOMMENT(); 

			}
			break;
		case 72:
			DebugEnterAlt(72);
			// Scrape.g:1:563: LINE_COMMENT_SLASH
			{
			DebugLocation(1, 563);
			mLINE_COMMENT_SLASH(); 

			}
			break;
		case 73:
			DebugEnterAlt(73);
			// Scrape.g:1:582: LINE_COMMENT_DASH
			{
			DebugLocation(1, 582);
			mLINE_COMMENT_DASH(); 

			}
			break;

		}

	}


	#region DFA
	DFA8 dfa8;

	protected override void InitDFAs()
	{
		base.InitDFAs();
		dfa8 = new DFA8(this, SpecialStateTransition8);
	}

	private class DFA8 : DFA
	{
		private const string DFA8_eotS =
			"\x2\xFFFF\x7\x19\x1\xFFFF\x1\x3B\x1\x3D\x1\xFFFF\xB\x19\x1\x4F\x8\xFFFF"+
			"\x1\x52\x1\x55\x4\xFFFF\x1\x56\x1\x57\xE\x19\x1\x68\x2\x19\x4\xFFFF\x1"+
			"\x6E\x10\x19\x9\xFFFF\x4\x19\x1\x85\xB\x19\x1\xFFFF\x1\x92\x3\x19\x1"+
			"\x96\x1\xFFFF\x13\x19\x1\xAA\x1\xAB\x1\x19\x1\xFFFF\x1\xAD\x1\xAE\x1"+
			"\xAF\x1\x19\x1\xB1\x1\x19\x1\xB3\x1\xB4\x1\x19\x1\xB6\x1\xB7\x1\xB8\x1"+
			"\xFFFF\x3\x19\x1\xFFFF\x3\x19\x1\xBF\x1\xC0\x2\x19\x1\xC3\x1\x19\x1\xC5"+
			"\x3\x19\x1\xC9\x5\x19\x2\xFFFF\x1\x19\x3\xFFFF\x1\x19\x1\xFFFF\x1\xD1"+
			"\x2\xFFFF\x1\x19\x3\xFFFF\x6\x19\x2\xFFFF\x1\x19\x1\xDA\x1\xFFFF\x1\xDB"+
			"\x1\xFFFF\x1\xDC\x2\x19\x1\xFFFF\x5\x19\x1\xE4\x1\xE5\x1\xFFFF\x3\x19"+
			"\x1\xFFFF\x1\x19\x1\xEA\x1\xEB\x1\x19\x3\xFFFF\x3\x19\x1\xF0\x2\x19\x1"+
			"\xF3\x2\xFFFF\x2\x19\x1\xFFFF\x1\x19\x2\xFFFF\x1\x19\x1\xFB\x2\x19\x1"+
			"\xFFFF\x2\x19\x1\xFFFF\x1\x100\x1\x19\x3\xFFFF\x1\x102\x1\x19\x1\xFFFF"+
			"\x2\x19\x1\x106\x1\x107\x1\xFFFF\x1\x108\x1\xFFFF\x2\x19\x4\xFFFF\x2"+
			"\x19\x2\xFFFF\x5\x19\x1\x114\x1\x19\x1\xFFFF\x1\x116\x1\xFFFF";
		private const string DFA8_eofS =
			"\x117\xFFFF";
		private const string DFA8_minS =
			"\x1\x9\x1\xFFFF\x1\x6E\x1\x69\x2\x61\x1\x68\x1\x61\x1\x6E\x1\xFFFF\x2"+
			"\x3D\x1\xFFFF\x1\x64\x1\x65\x1\x69\x2\x6F\x1\x61\x1\x65\x1\x74\x1\x6F"+
			"\x1\x75\x1\x6F\x1\x0\x8\xFFFF\x1\x2D\x1\x2A\x4\xFFFF\x2\x40\x1\x6F\x1"+
			"\x63\x1\x65\x1\x64\x1\x73\x1\x63\x1\x73\x2\x65\x1\x74\x1\x65\x1\x75\x1"+
			"\x6B\x1\x73\x1\x40\x1\x64\x1\x74\x4\xFFFF\x1\x40\x1\x65\x1\x6C\x1\x72"+
			"\x1\x6F\x1\x65\x1\x6F\x1\x69\x1\x64\x1\x6C\x1\x74\x1\x70\x1\x77\x1\x6D"+
			"\x1\x77\x1\x66\x1\x63\x9\xFFFF\x1\x63\x1\x6B\x1\x63\x1\x61\x1\x40\x1"+
			"\x65\x1\x68\x1\x65\x1\x61\x1\x6E\x1\x68\x2\x6E\x1\x65\x2\x74\x1\xFFFF"+
			"\x1\x40\x1\x72\x2\x65\x1\x40\x1\xFFFF\x1\x6E\x1\x65\x1\x69\x1\x6D\x1"+
			"\x65\x1\x6C\x1\x61\x1\x6E\x1\x65\x1\x6C\x1\x63\x1\x6C\x1\x74\x1\x6C\x1"+
			"\x6E\x1\x66\x1\x61\x1\x65\x1\x69\x2\x40\x1\x6E\x1\xFFFF\x3\x40\x1\x74"+
			"\x1\x40\x1\x65\x2\x40\x1\x63\x3\x40\x1\xFFFF\x1\x69\x2\x72\x1\xFFFF\x1"+
			"\x74\x1\x63\x1\x6E\x2\x40\x1\x64\x1\x74\x1\x40\x1\x73\x1\x40\x1\x68\x1"+
			"\x61\x1\x65\x1\x40\x1\x6C\x1\x65\x1\x74\x1\x64\x1\x65\x2\xFFFF\x1\x64"+
			"\x3\xFFFF\x1\x65\x1\xFFFF\x1\x40\x2\xFFFF\x1\x61\x3\xFFFF\x1\x62\x1\x74"+
			"\x1\x20\x1\x69\x1\x74\x1\x67\x2\xFFFF\x1\x74\x1\x40\x1\xFFFF\x1\x40\x1"+
			"\xFFFF\x1\x40\x1\x63\x1\x72\x1\xFFFF\x1\x6F\x1\x72\x1\x69\x1\x75\x1\x73"+
			"\x2\x40\x1\xFFFF\x1\x74\x1\x75\x1\x20\x1\xFFFF\x1\x74\x2\x40\x1\x65\x3"+
			"\xFFFF\x1\x65\x1\x6D\x1\x61\x1\x40\x1\x6F\x1\x72\x1\x40\x2\xFFFF\x1\x65"+
			"\x1\x74\x1\x66\x1\x79\x2\xFFFF\x1\x72\x1\x40\x1\x69\x1\x64\x1\xFFFF\x1"+
			"\x6E\x1\x65\x1\xFFFF\x1\x40\x1\x65\x3\xFFFF\x1\x40\x1\x6D\x1\xFFFF\x1"+
			"\x6E\x1\x20\x2\x40\x1\xFFFF\x1\x40\x1\xFFFF\x1\x69\x1\x61\x1\x69\x3\xFFFF"+
			"\x1\x6E\x1\x74\x2\xFFFF\x1\x61\x1\x6F\x1\x74\x1\x72\x1\x6F\x1\x40\x1"+
			"\x72\x1\xFFFF\x1\x40\x1\xFFFF";
		private const string DFA8_maxS =
			"\x1\x7D\x1\xFFFF\x2\x72\x1\x78\x1\x72\x1\x69\x1\x72\x1\x74\x1\xFFFF\x2"+
			"\x3D\x1\xFFFF\x1\x6E\x1\x74\x1\x72\x1\x6F\x1\x75\x1\x61\x1\x6F\x1\x74"+
			"\x1\x6F\x1\x75\x1\x6F\x1\xFFFF\x8\xFFFF\x1\x2D\x1\x2F\x4\xFFFF\x2\x7A"+
			"\x1\x6F\x1\x63\x1\x70\x1\x64\x1\x73\x1\x63\x1\x73\x2\x65\x1\x74\x1\x65"+
			"\x1\x75\x1\x6B\x1\x78\x1\x7A\x1\x64\x1\x74\x4\xFFFF\x1\x7A\x1\x65\x1"+
			"\x6C\x1\x72\x1\x6F\x1\x6C\x1\x6F\x1\x69\x1\x64\x1\x6C\x1\x74\x1\x70\x1"+
			"\x77\x1\x6D\x1\x77\x1\x66\x1\x63\x9\xFFFF\x1\x78\x1\x6B\x1\x63\x1\x61"+
			"\x1\x7A\x1\x65\x1\x68\x1\x65\x1\x61\x1\x72\x1\x68\x2\x6E\x1\x65\x2\x74"+
			"\x1\xFFFF\x1\x7A\x1\x72\x2\x65\x1\x7A\x1\xFFFF\x1\x6E\x1\x65\x1\x69\x1"+
			"\x6D\x1\x65\x1\x6C\x1\x61\x1\x6E\x1\x65\x1\x6C\x1\x63\x1\x6C\x1\x74\x1"+
			"\x6C\x1\x6E\x1\x66\x1\x61\x1\x65\x1\x69\x2\x7A\x1\x6E\x1\xFFFF\x3\x7A"+
			"\x1\x74\x1\x7A\x1\x65\x2\x7A\x1\x63\x3\x7A\x1\xFFFF\x1\x69\x2\x72\x1"+
			"\xFFFF\x1\x74\x1\x63\x1\x6E\x2\x7A\x1\x64\x1\x74\x1\x7A\x1\x73\x1\x7A"+
			"\x1\x68\x1\x61\x1\x65\x1\x7A\x1\x6C\x1\x65\x1\x74\x1\x64\x1\x65\x2\xFFFF"+
			"\x1\x64\x3\xFFFF\x1\x65\x1\xFFFF\x1\x7A\x2\xFFFF\x1\x61\x3\xFFFF\x1\x62"+
			"\x1\x74\x1\x20\x1\x69\x1\x74\x1\x67\x2\xFFFF\x1\x74\x1\x7A\x1\xFFFF\x1"+
			"\x7A\x1\xFFFF\x1\x7A\x1\x63\x1\x72\x1\xFFFF\x1\x6F\x1\x72\x1\x69\x1\x75"+
			"\x1\x73\x2\x7A\x1\xFFFF\x1\x74\x1\x75\x1\x20\x1\xFFFF\x1\x74\x2\x7A\x1"+
			"\x65\x3\xFFFF\x1\x65\x1\x6D\x1\x61\x1\x7A\x1\x6F\x1\x72\x1\x7A\x2\xFFFF"+
			"\x1\x65\x1\x74\x1\x6F\x1\x79\x2\xFFFF\x1\x72\x1\x7A\x1\x69\x1\x64\x1"+
			"\xFFFF\x1\x6E\x1\x65\x1\xFFFF\x1\x7A\x1\x65\x3\xFFFF\x1\x7A\x1\x6D\x1"+
			"\xFFFF\x1\x6E\x1\x20\x2\x7A\x1\xFFFF\x1\x7A\x1\xFFFF\x1\x69\x1\x61\x1"+
			"\x70\x3\xFFFF\x1\x6E\x1\x74\x2\xFFFF\x1\x61\x1\x6F\x1\x74\x1\x72\x1\x6F"+
			"\x1\x7A\x1\x72\x1\xFFFF\x1\x7A\x1\xFFFF";
		private const string DFA8_acceptS =
			"\x1\xFFFF\x1\x1\x7\xFFFF\x1\xC\x2\xFFFF\x1\x11\xC\xFFFF\x1\x38\x1\x39"+
			"\x1\x3A\x1\x3B\x1\x3C\x1\x3D\x1\x3E\x1\x3F\x2\xFFFF\x1\x42\x1\x43\x1"+
			"\x45\x1\x46\x13\xFFFF\x1\xE\x1\xD\x1\x10\x1\xF\x11\xFFFF\x1\x44\x1\x37"+
			"\x1\x49\x1\x40\x1\x47\x1\x48\x1\x41\x1\x2\x1\xB\x10\xFFFF\x1\x29\x5\xFFFF"+
			"\x1\x17\x16\xFFFF\x1\x8\xC\xFFFF\x1\xA\x3\xFFFF\x1\x32\x13\xFFFF\x1\x1E"+
			"\x1\x4\x1\xFFFF\x1\x9\x1\x16\x1\x5\x1\xFFFF\x1\x6\x1\xFFFF\x1\x30\x1"+
			"\x7\x1\xFFFF\x1\x1F\x1\x23\x1\x2B\x6\xFFFF\x1\x19\x1\x2E\x2\xFFFF\x1"+
			"\x1B\x1\xFFFF\x1\x2C\x3\xFFFF\x1\x24\x7\xFFFF\x1\x1D\x3\xFFFF\x1\x1A"+
			"\x4\xFFFF\x1\x33\x1\x1C\x1\x20\x7\xFFFF\x1\x28\x1\x2D\x4\xFFFF\x1\x18"+
			"\x1\x31\x4\xFFFF\x1\x2F\x2\xFFFF\x1\x2A\x2\xFFFF\x1\x12\x1\x13\x1\x14"+
			"\x2\xFFFF\x1\x21\x4\xFFFF\x1\x15\x1\xFFFF\x1\x27\x3\xFFFF\x1\x36\x1\x3"+
			"\x1\x22\x2\xFFFF\x1\x25\x1\x26\x7\xFFFF\x1\x35\x1\xFFFF\x1\x34";
		private const string DFA8_specialS =
			"\x18\xFFFF\x1\x0\xFE\xFFFF}>";
		private static readonly string[] DFA8_transitionS =
			{
				"\x2\x26\x2\xFFFF\x1\x26\x12\xFFFF\x1\x26\x1\xC\x1\x24\x1\xFFFF\x1\x1"+
				"\x2\xFFFF\x1\x18\x1\x1C\x1\x1D\x1\x1A\x1\x20\x1\x23\x1\x21\x1\x1B\x1"+
				"\x22\xA\x25\x2\xFFFF\x1\xA\x1\x9\x1\xB\x1\xFFFF\x1B\x19\x6\xFFFF\x1"+
				"\x8\x1\x16\x1\x5\x1\x15\x1\x4\x1\xF\x1\x19\x1\x14\x1\xD\x1\x10\x1\x19"+
				"\x1\x17\x1\x12\x1\x11\x1\x2\x1\x3\x1\x19\x1\x13\x1\xE\x1\x7\x2\x19\x1"+
				"\x6\x3\x19\x1\x1E\x1\xFFFF\x1\x1F",
				"",
				"\x1\x27\x3\xFFFF\x1\x28",
				"\x1\x2A\x8\xFFFF\x1\x29",
				"\x1\x2E\xA\xFFFF\x1\x2D\x1\xFFFF\x1\x2C\x9\xFFFF\x1\x2B",
				"\x1\x2F\x10\xFFFF\x1\x30",
				"\x1\x31\x1\x32",
				"\x1\x35\x3\xFFFF\x1\x36\x2\xFFFF\x1\x33\x6\xFFFF\x1\x37\x2\xFFFF\x1"+
				"\x34",
				"\x1\x38\x5\xFFFF\x1\x39",
				"",
				"\x1\x3A",
				"\x1\x3C",
				"",
				"\x1\x3F\x9\xFFFF\x1\x3E",
				"\x1\x40\xE\xFFFF\x1\x41",
				"\x1\x43\x2\xFFFF\x1\x44\x5\xFFFF\x1\x42",
				"\x1\x45",
				"\x1\x46\x5\xFFFF\x1\x47",
				"\x1\x48",
				"\x1\x49\x9\xFFFF\x1\x4A",
				"\x1\x4B",
				"\x1\x4C",
				"\x1\x4D",
				"\x1\x4E",
				"\x0\x50",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"\x1\x51",
				"\x1\x53\x4\xFFFF\x1\x54",
				"",
				"",
				"",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x58",
				"\x1\x59",
				"\x1\x5A\xA\xFFFF\x1\x5B",
				"\x1\x5C",
				"\x1\x5D",
				"\x1\x5E",
				"\x1\x5F",
				"\x1\x60",
				"\x1\x61",
				"\x1\x62",
				"\x1\x63",
				"\x1\x64",
				"\x1\x65",
				"\x1\x67\x4\xFFFF\x1\x66",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x69",
				"\x1\x6A",
				"",
				"",
				"",
				"",
				"\x1B\x19\x6\xFFFF\xD\x19\x1\x6C\x4\x19\x1\x6B\x1\x6D\x6\x19",
				"\x1\x6F",
				"\x1\x70",
				"\x1\x71",
				"\x1\x72",
				"\x1\x74\x6\xFFFF\x1\x73",
				"\x1\x75",
				"\x1\x76",
				"\x1\x77",
				"\x1\x78",
				"\x1\x79",
				"\x1\x7A",
				"\x1\x7B",
				"\x1\x7C",
				"\x1\x7D",
				"\x1\x7E",
				"\x1\x7F",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"",
				"\x1\x80\x14\xFFFF\x1\x81",
				"\x1\x82",
				"\x1\x83",
				"\x1\x84",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x86",
				"\x1\x87",
				"\x1\x88",
				"\x1\x89",
				"\x1\x8A\x3\xFFFF\x1\x8B",
				"\x1\x8C",
				"\x1\x8D",
				"\x1\x8E",
				"\x1\x8F",
				"\x1\x90",
				"\x1\x91",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x93",
				"\x1\x94",
				"\x1\x95",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1\x97",
				"\x1\x98",
				"\x1\x99",
				"\x1\x9A",
				"\x1\x9B",
				"\x1\x9C",
				"\x1\x9D",
				"\x1\x9E",
				"\x1\x9F",
				"\x1\xA0",
				"\x1\xA1",
				"\x1\xA2",
				"\x1\xA3",
				"\x1\xA4",
				"\x1\xA5",
				"\x1\xA6",
				"\x1\xA7",
				"\x1\xA8",
				"\x1\xA9",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xAC",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xB0",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xB2",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xB5",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1\xB9",
				"\x1\xBA",
				"\x1\xBB",
				"",
				"\x1\xBC",
				"\x1\xBD",
				"\x1\xBE",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xC1",
				"\x1\xC2",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xC4",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xC6",
				"\x1\xC7",
				"\x1\xC8",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xCA",
				"\x1\xCB",
				"\x1\xCC",
				"\x1\xCD",
				"\x1\xCE",
				"",
				"",
				"\x1\xCF",
				"",
				"",
				"",
				"\x1\xD0",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"",
				"\x1\xD2",
				"",
				"",
				"",
				"\x1\xD3",
				"\x1\xD4",
				"\x1\xD5",
				"\x1\xD6",
				"\x1\xD7",
				"\x1\xD8",
				"",
				"",
				"\x1\xD9",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xDD",
				"\x1\xDE",
				"",
				"\x1\xDF",
				"\x1\xE0",
				"\x1\xE1",
				"\x1\xE2",
				"\x1\xE3",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1\xE6",
				"\x1\xE7",
				"\x1\xE8",
				"",
				"\x1\xE9",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xEC",
				"",
				"",
				"",
				"\x1\xED",
				"\x1\xEE",
				"\x1\xEF",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xF1",
				"\x1\xF2",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"",
				"\x1\xF4",
				"\x1\xF5",
				"\x1\xF8\x2\xFFFF\x1\xF6\x5\xFFFF\x1\xF7",
				"\x1\xF9",
				"",
				"",
				"\x1\xFA",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\xFC",
				"\x1\xFD",
				"",
				"\x1\xFE",
				"\x1\xFF",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x101",
				"",
				"",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x103",
				"",
				"\x1\x104",
				"\x1\x105",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"",
				"\x1\x109",
				"\x1\x10A",
				"\x1\x10B\x6\xFFFF\x1\x10C",
				"",
				"",
				"",
				"\x1\x10D",
				"\x1\x10E",
				"",
				"",
				"\x1\x10F",
				"\x1\x110",
				"\x1\x111",
				"\x1\x112",
				"\x1\x113",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				"\x1\x115",
				"",
				"\x1B\x19\x6\xFFFF\x1A\x19",
				""
			};

		private static readonly short[] DFA8_eot = DFA.UnpackEncodedString(DFA8_eotS);
		private static readonly short[] DFA8_eof = DFA.UnpackEncodedString(DFA8_eofS);
		private static readonly char[] DFA8_min = DFA.UnpackEncodedStringToUnsignedChars(DFA8_minS);
		private static readonly char[] DFA8_max = DFA.UnpackEncodedStringToUnsignedChars(DFA8_maxS);
		private static readonly short[] DFA8_accept = DFA.UnpackEncodedString(DFA8_acceptS);
		private static readonly short[] DFA8_special = DFA.UnpackEncodedString(DFA8_specialS);
		private static readonly short[][] DFA8_transition;

		static DFA8()
		{
			int numStates = DFA8_transitionS.Length;
			DFA8_transition = new short[numStates][];
			for ( int i=0; i < numStates; i++ )
			{
				DFA8_transition[i] = DFA.UnpackEncodedString(DFA8_transitionS[i]);
			}
		}

		public DFA8( BaseRecognizer recognizer, SpecialStateTransitionHandler specialStateTransition )
			: base(specialStateTransition)
		{
			this.recognizer = recognizer;
			this.decisionNumber = 8;
			this.eot = DFA8_eot;
			this.eof = DFA8_eof;
			this.min = DFA8_min;
			this.max = DFA8_max;
			this.accept = DFA8_accept;
			this.special = DFA8_special;
			this.transition = DFA8_transition;
		}

		public override string Description { get { return "1:1: Tokens : ( T__110 | T__111 | PROCEDURE | EXEC | CASE | WHEN | THEN | END | ELSE | AND | OR | EQUALS | LESSTHAN | LESSTHANEQUAL | GREATERTHAN | GREATERTHANEQUAL | NOTEQUAL | INSERT_INTO | INSERT_OVERWRITE | INSERT_DIRECTORY | TRUNCATE | EACH | IN | SELECT | FROM | INNER_JOIN | JOIN | NODES | WHERE | PICK | TAKE | MATCH | REPLACE | ATTRIBUTE | TEXT | HTML | DOWNLOAD_IMAGE | DOWNLOAD_PAGE | IDENTITY | EXPAND | TO | PROXIES | TEST | NULL | CREATE | FILE | BUFFER | WITH | STRING | INTEGER | FLOAT | FIELD_TERMINATOR | ROW_TERMINATOR | LOCATION | STRING_LITERAL | ID | ASTERISK | DOT | OPENPAREN | CLOSEPAREN | OPENBRACE | CLOSEBRACE | PLUS | MINIS | DIV | COMMA | QUOTE | APOSTRAPHE | INT | WS | COMMENT | LINE_COMMENT_SLASH | LINE_COMMENT_DASH );"; } }

		public override void Error(NoViableAltException nvae)
		{
			DebugRecognitionException(nvae);
		}
	}

	private int SpecialStateTransition8(DFA dfa, int s, IIntStream _input)
	{
		IIntStream input = _input;
		int _s = s;
		s = -1;
		int LA8_1 = input.LA(1);
		int index8_1 = input.Index;
		switch (_s)
		{
		case 0:
			{
				if (((LA8_1>='\u0000' && LA8_1<='\uFFFF'))) {s = 80;}

				else s = 79;

				break;
			}

		default:
			break;
		}

		if (s >= 0)
			return s;

		NoViableAltException nvae = new NoViableAltException(dfa.Description, 8, _s, input);
		dfa.Error(nvae);
		throw nvae;
	}

	#endregion

}
